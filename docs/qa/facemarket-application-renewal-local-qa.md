...
## QA 시나리오
1. **지원**: `?facemarket=1` → 상단바 **"모델 지원하기"** → `model@local.test` 로그인 → 곧바로 `/model/apply`
   (허브 안 거침) → 프로필 사진 업로드 → 필드 입력 → 확인 2개·고지 동의 → 제출 → 상단바 **"등록 상태"**(`/status`)
   에서 "지원서를 검토하고 있어요". (`/model` 로 들어가도 `/status` 로 넘어온다. 라이선스 탭은 없어졌다.)
2. **검토**: 새 탭 `?admin=1` → `qa@local.test` 로그인 → 카드(사진·정보) → 승인 또는 거절(사유).
3. **반영**: 지원자 탭 `/status` 새로고침 → 승인 "지원이 승인됐어요"+"모델 등록 계속하기" / 거절 사유+"다시 지원하기".
   상단바 CTA 도 같이 바뀐다(검토 중=지원 상태 보기, 승인=모델 등록 계속하기, 거절=다시 지원하기).
