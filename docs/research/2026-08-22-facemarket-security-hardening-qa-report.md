...
- Browser cache boundary: frontend builds default to the legacy `e=1` capability. Only an explicit post-backend `VITE_ASSET_CACHE_VERSION=2` build emits `e=2`; sensitive `/file?e=2` then redirects to the API `/bytes?e=2` response with `private, no-store`, without appending unsigned query parameters to presigned R2 URLs. API asset capability URLs also bypass Cloudflare image transformations, while direct public images retain resizing.
...
- Allowed/forbidden category enforcement: forbidden categories take precedence, product-only, virtual, and ordinary vary paths do not enter real FaceMarket settlement/purge/cleanup paths, and retained/malicious real-model UUIDs are stripped from product-only requests before enqueue. A vary request sourced from a REAL-derived asset instead inherits only its server-resolved producer-job snapshot, rechecks the license, emits `private, no-store`, and remains discoverable by model/license purge scope.
...
8. Keep `VITE_ASSET_CACHE_VERSION` unset or `1` through backend deployment and smoke. Only afterward build and release the frontend with the value explicitly set to `2`; never expose `e=2` first.
