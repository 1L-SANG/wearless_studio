...
- **업로드 몰림 두 번**: `slow-response` 는 빈 구간을 건너뛰고 최근 실제 값 두 개로 판정한다. 가운데 빈 5분을 둔 업로드 몰림 두 번이 겹치면 울 수 있다(60일 동안 15분 안에 두 번은 0회). 근본 해결은 로드밸런서 중앙값 대신 앱이 업로드 같은 느린 경로를 뺀 응답 시간을 직접 지표로 내는 것이다(후속 과제).
- **INSUFFICIENT_DATA 를 거친 복구**: Slack 램다는 INSUFFICIENT_DATA 에서 OK 로 가는 알림을 보내지 않는다(새 알람의 첫 판정이 '복구'로 보이지 않게). 여기 알람들은 빈 데이터를 모두 따로 처리해서 INSUFFICIENT_DATA 는 생성 때만 생기지만, 지표 식 오류나 LB 교체로 거기를 거쳐 복구되면 그 복구 알림은 오지 않는다.
...
앱 재배포와 무관하다. 환경 애드온만 바뀐다. 두 곳을 같이 바꾼 변경은 **둘 다 반영해야 끝난다.** 한쪽만 나가면 옛 알람과 새 알람이 같이 돌거나 main 설명과 실제가 어긋난다. 순서는 머지 직후 환경 쪽 먼저다. Slack 램다 같은 공용 부분이 환경 쪽에 있어서, CI 가 api 를 배포하기(수 분 걸림) 전에 올라가 있어야 한다. ⚠️ **CF 원본 잠금(#315)과 순서가 얽힌다**: 그 작업도 `env deploy` 를 쓴다. 두 개를 같은 시점에 밀면 어느 쪽이 깨졌는지 못 가른다. 하나씩.
...
- 급할 때 배포 없이 잠시 끄기: `aws cloudwatch disable-alarm-actions --region us-east-1 --alarm-names <이름>`. 다음 배포 때 템플릿 값으로 돌아오니 원인을 고쳐 템플릿에도 반영한다
