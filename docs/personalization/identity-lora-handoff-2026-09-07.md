...
## 32. 마무리 측정 3종 — 해상도 · 톤 일관성 · P1 에 얼굴 패스 (2026-09-10)

모델 id 확인: `resolve_editor_cut_model = 'gpt-image-2.5-flare'` (정확히 일치).

### 32.A 해상도 — **2K·4K 둘 다 된다**
P1 zara2 를 같은 입력·프롬프트로 `image_size` 만 바꿔 각 1회. 에러 없음.

| 요청 | 실제 크기 | 얼굴폭 |
|---|---|---|
| ZARA 원본 | 2048×3072 | 322 |
| `1K` | 1024×1536 | 164 |
| `2K` | **1536×2304** | 239 |
| `4K` | **2336×3504** | **350** |

4K 는 원본보다 크고 얼굴 크롭에서 피부 디테일이 원본급이다(`studio/size_face_crop.png`).
→ §31.3 의 해상도 미결은 닫힌다. 코드 변경 없이 호출부에서 `image_size="4K"` 를 넘기면 된다.

### 32.B 톤 일관성 — 강제 ESRGAN ×2 + 얼굴 패스 — **불채택(규칙 초과 1건)**
`AUTO_UPSCALE_FACE_W_MIN` 은 얼굴폭<120px 일 때만 켜져서 gpt-image 컷(얼굴 200px 대)에는 안 붙는다.
**모듈 상수는 그대로 두고** 러너가 1단계에서 입력을 ESRGAN ×2 로 먼저 확대한 뒤 패스를 걸었다
(1024×1536 → 2048×3072, 얼굴폭 219~427). 5/5 첫 시드 통과.

| 컷 | SFace 전 | SFace 후 | Δ | 매끈함 전 | 매끈함 후 |
|---|---|---|---|---|---|
| front | 0.842 | 0.804 | −0.038 | 2.92 | 3.33 |
| three_quarter | 0.836 | 0.746 | **−0.090** | 4.64 | 3.21 |
| full | 0.803 | 0.823 | +0.020 | 4.65 | 4.10 |
| seated | 0.856 | 0.844 | −0.012 | 3.90 | 3.63 |
| close | 0.877 | 0.817 | −0.060 | 4.12 | 3.28 |
| [어제] m9_3_E2 | — | 0.784 | — | — | 2.97 |
| [어제] m9_5_E2 | — | 0.736 | — | — | 3.27 |

**육안은 통과다** — 5장이 어제 좋아한 톤과 같은 계열이고(매끈한 피부·광고 사진 느낌) 서로도 일관된다.
매끈함 지표도 3.21~4.10 으로 어제(2.97·3.27)와 겹친다. **그러나 SFace 문턱을 하나가 넘겼다**:
`three_quarter` −0.090 > −0.08. 판정 규칙("5장 모두 육안 톤 계열 **AND** SFace −0.08 이내")대로 **불채택**.
규칙을 사후에 옮기지 않는다. 나머지 4장은 −0.060 ~ +0.020 으로 통과했다.

→ "ESRGAN 조건부 → 항상 1회" 코드 변경은 **제안하지 않는다.** 다시 하려면 `three_quarter` 가 왜 혼자
−0.09 인지부터 봐야 한다(얼굴폭 365.9 로 다른 컷과 비슷한데 유독 떨어졌다 — 원인 미확인).

### 32.C P1 결과에 얼굴 패스 — **①~⑤ 통과(채택), 다만 걸 이유가 없다**
| 컷 | SFace | 줄무늬 | 배경 \|Δ\| | 매끈함 |
|---|---|---|---|---|
| zara2 P1 원본 | **0.840** | 0.4246 | 8.9 | 2.85 |
| zara2 C1 기본 | 0.782 | 0.4370 | 9.3 | 3.25 |
| zara2 C2 강제 | 0.826 | 0.4358 | 9.0 | 3.89 |
| zara1 P1 원본 | **0.807** | 0.4032 | 6.4 | 5.77 |
| zara1 C1 기본 | 0.726 | 0.3901 | 7.1 | 3.48 |
| zara1 C2 강제 | 0.726 | 0.3901 | 7.1 | 3.48 |

- ① 머리 위 사각 **없음** · ② 잘린 벽 그림자 **없음** · ③ 어깨 얼룩 **없음** — 확대(`bc_measure/C_zoom.png`)에서
  P1·C1·C2 모두 벽 그림자가 자연스럽게 이어진다. **흰 스튜디오 벽에서도 세 증상이 재현되지 않았다.**
  이전 경로 B 는 ZARA **원본**(금발·다른 사람)에 패스를 걸어 색 보정이 크게 밀렸는데, 이번엔 이미 우리 인물인
  P1 결과에 거는 것이라 보정량이 작다.
- ④ SFace ≥0.70 ✓ (0.726~0.826) · ⑤ 옷·배경 유지 ✓ (줄무늬 ±0.013, 배경 |Δ| 6.4→7.1 · 8.9→9.3)

→ 규칙상 **채택**이고 C1·C2 중에서는 zara2 에서 C2 가 높다(0.826 vs 0.782), zara1 은 동일.
**그런데 두 팔 모두 패스 전 P1(0.840·0.807)보다 낮다.** §26.5 와 같은 결론이다 — gpt-image 가 이미 인물을
재현했으면 그 위에 LoRA 얼굴을 덮어도 좋아지지 않는다. 이음선이 안 생긴다는 것만 확인됐고, **거는 이득은 없다.**

`zara1` 은 C1·C2 가 완전히 같은 값이다. P1 결과의 얼굴폭이 100.6px 이라 C1 에서도 모듈의 자동 확대가
스스로 켜져(임계 120) 두 팔이 같은 입력이 됐다 — 설계대로다.

### 32.D 정리
- 해상도: `image_size="4K"` 로 닫힘.
- 톤: 강제 ESRGAN 은 육안으로는 되는데 규칙을 하나가 넘겨 불채택. 원인 미확인.
- P1 에 패스: 이음선은 안 생기지만 신원이 내려가므로 **걸지 않는 쪽이 낫다.**
