...
3. **작업권(takeover) 방식.** '이어서'를 누른 기기가 작업권을 가져가고, 다른 기기는 다음 저장/동작에서 서버가 거부 → 잠금 화면("다른 기기에서 이어서 작업을 시작했어요 · [여기서 다시 이어받기]"). 항상 한 기기만 편집. 미저장 로컬 변경이 있는 채 이어받으면 양쪽 저장 시각을 보여주고 기준을 선택하게 한다.
...
- 사진: 추가 즉시 백그라운드 업로드(변환된 JPEG). 실패는 조용히 재시도하고 슬롯 메타에 `photosPending` 표시 — 다른 기기에서 열면 "일부 사진은 아직 동기화 중"을 보여준다. **로컬 IndexedDB draft 가 항상 1차 진실**이며 서버 슬롯은 백업/이동용.
...
- 새 모듈 `src/lib/draftSlot.js`: token(localStorage, 기기 범위)·PUT 큐(직렬)·409 핸들러·takeover·deviceLabel 추정.
- 잠금 화면: ProductInput 위 전면 오버레이. "다른 기기에서 이어서 작업을 시작했어요 (N분 전 · {기기}) — 이 화면에서는 편집할 수 없어요" + [여기서 다시 이어받기]. 이어받기 → takeover → 원격 payload 로드. **미저장 로컬 변경이 있으면** "이 기기 내용(HH:MM) / 다른 기기 내용(HH:MM)" 선택 모달을 먼저.
