...
| 배경 | **시드 회색(≈232) 합성**, 불투명 저장 | 시드 카탈로그와 나란히 놨을 때 완전 동일. 카드 CSS 가 바뀌어도 일관 |
...
각 업로드 원본 → sam_client.segment_garment (투명 RGBA 컷아웃)
   ↓
시드 회색(≈232) 배경 합성 → 불투명 PNG → R2 (derived 경로)
   ↓
...
### matching_cutout 서비스
- `flatten_on_bg(rgba_png) -> opaque_png` — 투명 컷 위에 시드 회색(≈232) 배경. 배경색은
  상수 하나(시드 카탈로그 모서리색에서 측정한 값)로 고정.
...
- 서비스: `flatten_on_bg` 가 불투명·소스 크기, 배경색=시드 회색, 파생 신원 결정론.
