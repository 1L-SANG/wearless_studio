...
### 방향 결정 방식 (2026-08-07 오너 결정 — 리뷰 후 개정)

디테일 컷의 방향은 **셀러가 고르지 않는다.** 방향 선택 칩은 디테일 컷에서 숨긴 상태를 유지하고,
셀러가 고른 **생성예시의 `direction` 라벨**(미기재=front)이 `block.direction`을 내부 결정한다
(`generationExampleSelectionPatch` 첫 선택·교체, `assignGenerationExamples` 자동 배정 공통).
뒷면 디테일 예시(백넥·등판 클로즈업)에 `direction: 'back'` 라벨을 다는 것은 카탈로그 큐레이션
작업(오너)이다. 근거: 예시가 연출의 정본이라는 기존 문법과 일치 — 셀러에게 방향이라는
내부 개념을 노출하지 않는다.
...
- 콘티보드 기본 구성에 뒷면 디테일 컷 **기본 포함** 여부 — 별도 결정(뒷면 디테일 예시를 자동 배정 풀에 넣을지의 문제로 환원됨).
- 뒷면 디테일 생성예시의 `direction: 'back'` 라벨링 — 카탈로그 큐레이션(오너 작업), 현재 디테일 예시 9종은 라벨 미기재(=front 취급).
- 개편 전 시드된 기본 콘티의 지문 마이그레이션 — 기존 프로젝트가 전부 테스트용이라 하지 않기로 확정(2026-08-07 오너 결정). 옛 기본 시드는 "편집본"으로 남아 사진 양 변경에 반응하지 않는다.
