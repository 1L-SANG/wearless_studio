...
### 10.1 배포 순서 — 마이그레이션이 앱 DB 를 먼저 봐야 한다

`write_audit`(4.4)는 각 액션의 트랜잭션 안에서 실행되고 commit 하지 않는다 — INSERT 가
실패하면 액션 전체가 롤백된다. 이건 의도된 안전장치다: 기록 없이 커밋되는 액션이 없다.
그런데 이 안전장치는 뒤집으면 배포 순서 요구가 된다. `admin_audit_log`
(`20260904100000_admin_audit_log.sql`)가 **앱이 실제로 붙는** 데이터베이스에 없는
채로 백엔드 이미지가 먼저 배포되면, 지원서 승인/거절/재발송·환불 승인/거절 — 오늘
잘 도는 라우트 다섯 개가 배포 직후부터 전부 500 을 내고 아무 것도 처리하지 못한다.

순서: 이 마이그레이션을 앱 DB 에 먼저 적용 → 그 다음 백엔드 이미지 배포. "마이그가
CI 를 통과했다"는 "앱 DB 에 있다"의 증거가 아니다 — 이 저장소엔 CI 의
`SUPABASE_DB_URL` 이 앱 DB 와 다른 데이터베이스를 가리켜 마이그가 prod 에 안 붙은
전례가 있다(`ci-migration-wrong-db`). 실패 모드는 조용하지 않다: 5xx 가 즉시 뜨고
로그에 남는다 — 대신 사후 복구가 아니라 사전에 막을 수 있는 문제다.
...
1. **서버 DB 롤이 `auth.users` 를 읽는가.** 앱 코드에 선례가 없다. 막히면 `profiles` 이메일 미러 + 가입 트리거 수정으로 우회(6절).
   - **미확인 (2026-09-04, Task 8/11/13 구현 시점).** 이 작업은 프런트 전용 worktree(`wearless_studio-admin-ui`)에서 진행했고, Task 9 Step 5(서버 DB 롤 확인)는 병렬 백엔드 브랜치(`wearless_studio-admin-console`)의 작업 범위라 이 worktree에는 그 결과가 없다. 서버가 없어 직접 쿼리도 실행하지 못했다(작업 지시가 백엔드 기동·통합 테스트를 명시적으로 금지했다). 백엔드 트랙 완료 시 이 항목을 확정값으로 갱신해야 한다.
2. **`fm_models.user_id` 가 null 인 모델**(플랫폼 대행 온보딩)이 실제로 있는지. 있으면 목록의 '계정 이메일' 칸이 비고, 이메일 검색에 안 걸린다 — 표시 규칙을 정해야 한다.
   - **미확인.** 같은 이유로 DB 접근이 없다. `AdminModels.jsx` 상세 패널은 이미 `model.email || '연결된 계정 없음 (플랫폼 온보딩)'`으로 null 케이스를 가정하고 방어해 두었지만, 실제로 그런 행이 몇 개인지·표시 문구가 맞는지는 백엔드 확인 후 검증이 필요하다.
3. **prod 관리자 계정이 현재 몇 개인지.** 1개뿐이면 '최후 관리자 강등 금지' 가드가 배포 직후 곧바로 발동한다(정상 동작이지만, 두 번째 관리자를 먼저 만들어 두는 편이 낫다).
   - **미확인.** 같은 이유로 prod DB 에 접근하지 않았다. 배포 전 반드시 확인 — 1개뿐이면 두 번째 관리자를 먼저 만들어 두는 편이 이 화면의 '최후 관리자' 가드에 바로 걸리지 않는다.

---

## 14. 머지 후로 미룬 것 (전체 리뷰 2026-09-04)

전체 브랜치 리뷰의 Minor 9건은 고치지 않고 남겼다. 전문은
`docs/superpowers/reviews/2026-09-04-facemarket-admin-console-final-review.md`.

| # | 내용 |
|---|---|
| M-1 | 모델 상세의 "연결된 계정 없음 (플랫폼 온보딩)" 이 `email` 이 아니라 `user_id` 를 봐야 한다 |
| M-2 | 큐 카드 2개가 그 숫자를 설명해 주지 않는 화면으로 이동한다 |
| M-3 | 응답의 `period` 를 아무 화면도 읽지 않는다 |
| M-4 | 모델 목록이 50건에서 잘리는데 페이지네이션도, 잘렸다는 신호도 없다 |
| M-5 | 설계가 명시한 컬럼 2개가 화면에 없다 |
| M-6 | "`admin_guard` 만 `repo.is_admin` 을 부른다" 규칙을 하드코딩된 파일 목록으로 검사한다 — 새 파일은 그물 밖이다 |
| M-7 | `AdminApplications` 가 `?status=` 를 검증 없이 한 번만 읽는다 |
| M-8 | `set_role` 이 아무것도 바꾸지 않은 호출에도 감사 행을 쓴다 |
| M-9 | `package.json` 의 `pnpm.onlyBuiltDependencies` 가 왜 있는지 그 자리에 설명이 없다(실제 동작은 `pnpm-workspace.yaml` 이 한다) |
