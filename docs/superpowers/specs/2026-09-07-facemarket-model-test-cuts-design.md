...
7. `awaiting_confirm`에서는 새 생체 등록을 시작할 수 없다. 서버는 모델 행 잠금 안에서 409 `model_confirmation_required`로 거절하고, 등록 화면과 열린 완료 화면의 재등록 버튼은 `/model/confirm`으로 안내한다. 확정 후 `verified` 모델의 재등록과 라이선스 해지 후 재발급은 계속 허용한다.
8. 관리자 정지 기능은 유지하며, 확인 대기 중 정지했다가 해제하면 `awaiting_confirm`을 복원한다.
