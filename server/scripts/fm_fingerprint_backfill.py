...
2026-09-27: 쇼핑몰 썸네일 크롭 변형(kind='cut_crop') — 'cut' 행은 있는데 크롭 행이 없는 컷을 따로 채운다
(--only crops). 순찰·모델 제보가 정사각으로 잘린 대표 이미지를 컷에 잇는 데 쓴다.
...
select r.id::text as id, a.r2_bucket, a.r2_key
 ...
 where a.r2_key is not null
   and not exists (select 1 from fm_image_fingerprints f
                    where f.output_record_id = r.id and f.kind = 'cut_crop')
 order by r.created_at
 limit %s
"""
...
INSERT_CUT_CROP = (
    ...
    "(output_record_id, kind, region_y0, region_y1, phash, dhash) "
    "values (%s, 'cut_crop', %s, %s, %s, %s) on conflict do nothing"
)
...
def backfill_cut_crops(conn, r2_for, *, limit: int, apply: bool) -> tuple[int, int]:
    """썸네일 크롭 변형만. 정사각·가로 컷은 크롭이 0개라 다음 실행에도 대상에 남는다(쓰기 없음)."""
    with conn.cursor() as cur:
        cur.execute(CUT_CROPS_SQL, (limit,))
        todo = cur.fetchall()
    ok = failed = 0
    rows_total = 0
    for row in todo:
        ...
        if fp is None:
            ...
            continue
        ok += 1
        rows_total += len(fp["crops"])
        if apply and fp["crops"]:
            with conn.cursor() as cur:
                cur.executemany(INSERT_CUT_CROP, [
                    (row["id"], c["region_y0"], c["region_y1"],
                     fm_fingerprint.to_signed(c["phash"]), fm_fingerprint.to_signed(c["dhash"]))
                    for c in fp["crops"]
                ])
            conn.commit()
    print(f"[cut_crops] 대상 {len(todo)} · 계산 {ok} (크롭 {rows_total}행) · 실패 {failed}")
    return ok, failed
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--only", choices=("all", "cuts", "crops", "publications"), default="all")
    ...
    with psycopg.connect(s.database_url, row_factory=dict_row) as conn:
        ...
        if args.only in ("all", "crops"):
            failed += backfill_cut_crops(conn, r2_for, limit=args.limit, apply=args.apply)[1]
