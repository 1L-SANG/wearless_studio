...
**결론: v2 는 서빙하지 않는다(2026-07-31 롤백).** 벗은 베이스만 놓고 보면 v2 비율이 낫지만,
실제 착장컷(스트라이프 상의 + 부츠컷 데님)을 나란히 보니 원본 쪽이 나았다 — 옷이 다리를
덮으면 베이스의 과한 다리 길이가 드러나지 않고, 오히려 v2 는 컷 전체가 뭉툭해 보였다.
"베이스 단독 비교"로 판단하면 안 된다는 게 이 실험의 교훈이다. R2 객체
`seed/mannequin/base-women-2K-v2.png` 와 asset 행 `aa88e8cd-…` 는 남겨 뒀으니,
다시 쓰려면 MANNEQUIN_BASE_WOMEN_ASSET_ID 만 그 id 로 바꾸면 된다.
