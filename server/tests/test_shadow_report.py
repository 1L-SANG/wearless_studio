...
def test_an_empty_pipeline_reports_zero_not_a_crash():
    out = sr.report(_sv.distribution_dataset([row(source_kind="editor_asset")]))
    assert out["pipelines"]["mannequin_edit"]["samples"] == 0
    # 빈 파이프라인의 판정 자체는 순수 함수로 본다(리포트 상태와 섞지 않는다).
    assert sr._verdict([])["status"] == "insufficient_data"


# ── 표본 부족 ───────────────────────────────────────────────────────────────

def test_small_samples_are_marked_insufficient():
    out = sr.report(_sv.distribution_dataset([row() for _ in range(5)]))["pipelines"]["editor_vary"]
...
def test_no_rows_yield_no_invented_numbers():
    out = sr.report(_sv.distribution_dataset([]))
...
def test_metric_distribution_reports_n_per_axis():
    rows = [row(edit_qc_result=qc(metrics={"delta": {"hemY": i / 100}}))
            for i in range(10)]
    md = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["metricDistributions"]
...
def test_machine_pass_rejected_by_a_human_is_a_false_pass_candidate():
    ...
    c = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["confusion"]
...
def test_unreviewed_rows_are_not_graded():
    ...
    p = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]
...
def test_accepted_rate_uses_only_decided_rows():
    ...
    u = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["userReview"]
...
def test_vision_unavailability_rate_counts_every_non_ok_status():
    ...
    v = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["visionAvailability"]
...
def test_measurement_vision_conflict_is_counted():
    ...
    c = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["measurementVisionConflict"]
...
def test_null_observation_is_not_a_conflict():
    ...
    assert sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"][
...
def test_violations_are_counted_per_axis():
    ...
    v = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["violations"]
...
def test_cost_is_zero_until_unit_prices_are_given():
    ...
    prov = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["provider"]
    assert prov["estimatedUsd"] == 0.0 and prov["unitPricesProvided"] is False
    priced = sr.report(_sv.distribution_dataset(rows), image_usd=0.1)["pipelines"]["editor_vary"]["provider"]
...
def test_latency_skips_unfinished_sessions():
    ...
    lat = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["latencySeconds"]
...
def test_unreviewed_bulk_never_reaches_enforce_candidate():
    ...
    v = sr._verdict(_many(500, edit_qc_result=qc(decision="pass")))
    assert v["status"] == "insufficient_data"


def test_vision_unavailable_alone_blocks_enforce():
    """Vision 이 없으면 자동 통과 금지 — 표본이 아무리 많아도."""
    v = sr.report(_sv.distribution_dataset(_many(80, human_label="fidelity_pass",
...
def test_unknown_pipeline_is_reported_not_absorbed():
    out = sr.report(_sv.distribution_dataset([row(source_kind=None, id="x1"), row(source_kind="editor_asset")]))
...
def test_six_background_only_are_not_made_sufficient_by_custom_samples():
    ...
    # 판정 자체는 순수 함수로 본다 — 리포트 상태(distribution_only)와 섞지 않는다.
    assert sr._verdict([r for r in rows if r["edit_type"] == "BACKGROUND_ONLY"]
...
def test_each_edit_type_reports_its_own_metrics():
    ...
    d = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["byEditTypeDetail"]
...
def test_pass_samples_without_labels_block_enforce():
    ...
    v = sr._verdict([r for r in rows if r.get("edit_type") == "BACKGROUND_ONLY"],
                    is_enforce_eligible="BACKGROUND_ONLY" not in sr.NEVER_AUTO_PASS_TYPES)
    assert v["enforceReady"] is False
...
def test_human_label_coverage_counts_pass_samples_separately():
    rows = (_many(3, edit_qc_result=qc(decision="pass"), human_label="fidelity_pass")
            ...
            + _many(5, edit_qc_result=qc(decision="review_required"),
                    review_decision="accepted"))
    h = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["humanLabels"]
...
def test_provider_cost_uses_attempted_counters():
    ...
    prov = sr.report(_sv.distribution_dataset(rows), image_usd=0.1, vision_usd=0.01)["pipelines"][
...
def test_failed_samples_still_cost_money():
    rows = [row(output_id=None, edit_qc_result={"error": "x"}, image_calls=1,
                vision_calls=0)]
    prov = sr.report(_sv.distribution_dataset(rows), image_usd=0.1)["pipelines"]["editor_vary"]["provider"]
...
def test_pattern_axis_shows_unknown_in_the_report():
    out = sr.report(_sv.distribution_dataset([row(analysis={"clothingType": "top"}) for _ in range(3)]))
...
def test_an_invalid_manifest_marks_the_whole_report_unusable():
    out = sr.report(_sv.unverified_dataset(
        [row()], {"validForCalibration": False}, ["empty_allowed_scope"]))
    assert out["calibrationUsable"] is False
    # 상태 자체(manifest_unverified)도 사유로 남는다 — 왜 못 쓰는지가 하나라도
    # 빠지면 무엇부터 고칠지 알 수 없다.
    assert "empty_allowed_scope" in out["calibrationBlockedReasons"]
    assert "manifest_unverified" in out["calibrationBlockedReasons"]


def test_a_valid_manifest_does_not_add_a_block_flag():
    out = sr.report(_sv.distribution_dataset([row()]))
    # manifest 없이 낸 리포트는 언제나 분포다 — usable 은 닫혀 있다.
    assert out["calibrationUsable"] is False
...
def test_blinded_labels_count_toward_calibration_graded():
    ...
    cal = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["calibrationConfusion"]
...
def test_production_review_decisions_alone_cannot_satisfy_readiness():
    ...
    p = sr.report(_sv.distribution_dataset(rows))["pipelines"]["editor_vary"]["byEditTypeDetail"]["BACKGROUND_ONLY"]
