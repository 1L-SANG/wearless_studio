...
def test_an_empty_pipeline_reports_zero_not_a_crash():
    out = sr.report([row(source_kind="editor_asset")], manifest_verification=_trusted())
...
def test_metric_distribution_reports_n_per_axis():
    rows = [row(edit_qc_result=qc(metrics={"delta": {"hemY": i / 100}}))
            for i in range(10)]
    md = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["metricDistributions"]
...
def test_machine_pass_rejected_by_a_human_is_a_false_pass_candidate():
    ...
    c = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["confusion"]
...
def test_unreviewed_rows_are_not_graded():
    ...
    p = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]
...
def test_accepted_rate_uses_only_decided_rows():
    ...
    u = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["userReview"]
...
def test_vision_unavailability_rate_counts_every_non_ok_status():
    ...
    v = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["visionAvailability"]
...
def test_measurement_vision_conflict_is_counted():
    ...
    c = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["measurementVisionConflict"]
...
def test_null_observation_is_not_a_conflict():
    ...
    assert sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"][
...
def test_violations_are_counted_per_axis():
    ...
    v = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["violations"]
...
def test_cost_is_zero_until_unit_prices_are_given():
    ...
    prov = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["provider"]
...
def test_latency_skips_unfinished_sessions():
    ...
    lat = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["latencySeconds"]
...
def test_a_single_false_pass_blocks_enforce_even_with_many_samples():
    ...
    v = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["verdict"]
...
def test_unreviewed_bulk_never_reaches_enforce_candidate():
    ...
    v = sr.report(_many(500, edit_qc_result=qc(decision="pass")), manifest_verification=_trusted())[
...
def test_each_edit_type_reports_its_own_metrics():
    ...
    d = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["byEditTypeDetail"]
...
def test_pass_samples_without_labels_block_enforce():
    ...
    v = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["byEditTypeDetail"][
...
def test_human_label_coverage_counts_pass_samples_separately():
    rows = (_many(3, edit_qc_result=qc(decision="pass"), human_label="fidelity_pass")
            ...
            + _many(5, edit_qc_result=qc(decision="review_required"),
                    review_decision="accepted"))
    h = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["humanLabels"]
...
def test_an_invalid_manifest_marks_the_whole_report_unusable():
    out = sr.report([row()], manifest_verification=_sv.unverified({'validForCalibration': False}, ["empty_allowed_scope"]),)
...
def test_a_valid_manifest_does_not_add_a_block_flag():
    out = sr.report([row()], manifest_verification=_trusted())
...
def test_blinded_labels_count_toward_calibration_graded():
    ...
    cal = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["calibrationConfusion"]
...
def test_production_review_decisions_alone_cannot_satisfy_readiness():
    ...
    p = sr.report(rows, manifest_verification=_trusted())["pipelines"]["editor_vary"]["byEditTypeDetail"]["BACKGROUND_ONLY"]
