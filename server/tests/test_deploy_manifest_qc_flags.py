...
def test_api_addon_parameters_are_declared_by_an_addon_template():
    """addons.parameters.yml 의 값은 addon 템플릿 Parameters 와 짝이 맞아야 한다.
    한쪽만 지우거나 이름을 바꾸면 PR CI 는 통과하고 main 의 copilot svc deploy 에서야 깨진다."""
    ...
    declared = set()
    for tpl in addons_dir.glob("*.yml"):
        if tpl.name == "addons.parameters.yml":
            continue
        declared |= set(yaml.load(tpl.read_text(encoding="utf-8"), Loader=_CfnLoader).get("Parameters") or {})
    assert set(params["Parameters"]) <= declared
