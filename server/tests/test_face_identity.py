...
def test_expanded_ellipse_is_the_default_and_matches_the_formula():
    assert fi.ELLIPSE == (-0.55, -1.10, 1.55, 1.25)
    assert fi.ELLIPSE_TRAIN == (-0.30, -0.60, 1.30, 1.15)
    plan = fi.plan_from_box(848, 1264, (355.0, 252.0, 135.0, 180.0))
    fb = plan.face_box_crop
    assert list(plan.ellipse) == pytest.approx([fb[0] - 0.55 * fb[2], fb[1] - 1.10 * fb[3], fb[0] + 1.55 * fb[2], fb[1] + 1.25 * fb[3]])
    ...
    # 확장 타원이 더 넓다
    assert (np.asarray(fi.ellipse_mask(plan)) > 127).sum() > (np.asarray(fi.ellipse_mask(train)) > 127).sum()
    # 페더 기본은 0.12
