...
def test_accepts_new_and_previous_consent_versions():
    ...
    # 현재 버전은 2026-09-v3 다. #298 이 수집 항목을 "얼굴 8·상반신 5·전신 5" → "얼굴 16장"
    # 으로 바꾸면서 v2 로, 2026-09-15 에 좌·우 옆모습과 뒷모습이 더해지고("얼굴 16장" →
    # "등록 사진 18장") 사진 확인·사용 시점 두 항이 생기면서 v3 으로 올렸다 — 같은 버전
    # 문자열에 다른 본문을 게시하면 누가 어느 본문에 동의했는지 증명할 수 없다.
    # 2026-09-18 v4: 학습 주체(회사 직접)·GPU 서버 위탁·보관·파기 범위가 동의문에 더해졌다.
    assert BIOMETRIC_CONSENT_VERSION == "2026-09-v4"
