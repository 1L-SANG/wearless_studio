...
def test_build_fit_profile_block_full_profile():
    ...
    assert block == (
        ...
        "- length: a lengthened version of the same trousers whose hem falls past the ankle "
        "and reaches the top of the foot; if the photographed garment stops at or above the "
        "ankle bone, re-tailor only its length proportions by extending the leg hems down "
        "over the instep; preserve the photographed leg width and fabric behavior, letting "
        "any folds follow body contact naturally without a prescribed fold count; if it "
        "already satisfies this hem position, preserve those proportions. Observable target: "
        "both hems extend past and fully cover the ankle bones, reaching the tops of the "
        "feet; judge the hem endpoint, not a required crease or fold.\n"
