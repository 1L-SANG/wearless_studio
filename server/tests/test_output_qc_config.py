...
def test_mannequin_image_size_defaults_to_1k_everywhere(monkeypatch):
    """로컬, 테스트, 운영 모두 Sunburst 최초 생성의 native 1K 기본값을 쓴다."""
    monkeypatch.delenv("MANNEQUIN_IMAGE_SIZE", raising=False)
    assert Settings.__dataclass_fields__["mannequin_image_size"].default == "1K"
    assert load_settings().mannequin_image_size == "1K"

    monkeypatch.setenv("MANNEQUIN_IMAGE_SIZE", "invalid")
    assert load_settings().mannequin_image_size == "1K"


def test_default_detail_cut_resolution_stays_2k_when_mannequin_moves_to_1k(monkeypatch):
    monkeypatch.delenv("MANNEQUIN_IMAGE_SIZE", raising=False)
    monkeypatch.delenv("DETAIL_CUT_IMAGE_SIZE", raising=False)

    settings = load_settings()

    assert Settings.__dataclass_fields__["detail_cut_image_size"].default == "2K"
    assert settings.mannequin_image_size == "1K"
...
def test_production_manifest_bounds_4k_gpt_repair_concurrency_without_moving_shared_tier():
    ...
    assert variables["MANNEQUIN_IMAGE_SIZE"] == "1K"
    assert variables["MANNEQUIN_LOGO_IMAGE_SIZE"] == "off"
