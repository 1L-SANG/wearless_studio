...
def test_status_waits_for_license_did_then_issue_activates_and_gates_open():
    async def scenario(conn):
        ...
        return (waiting, not_claimed, pending, claimed, activated,
                ...
                await svc.has_active_credential(conn, model_id),
                await svc.public_view(conn, model_id))
    ...
    assert set(public) == {"active", "vc_id", "consented_at", "consent_doc_version"}
...
def test_close_revokes_and_enqueues_sponsorship_revocation():
    async def scenario(conn):
        ...
        return (closed, jobs, await svc.has_active_credential(conn, model_id),
                await svc.public_view(conn, model_id), await svc.owner_status(conn, model_id))
