...
def _setup(
    ...
    **settings_overrides,
    ...
    overrides = dict(
        ...
        # 같은 PR 이 필수 사진을 18장으로 늘렸고 17칸 스펙이 그걸 다시 바꿨다. 이 파일은
...
class AdminFakeCursor:
    ...
    async def execute(self, sql, params=None):
        ...
        if "fm_biometric_enrollment_photos" in query and query.startswith(
            ...
            # 17칸 스펙 이후 행 이름은 등록 회차마다 다르다 — 라우트가 후보 목록을 넘기고
...
def test_image_route_finds_the_17_slot_row_behind_the_old_name(admin_client):
    """심사 화면의 이름(정면·45도·측면)은 그대로지만 저장된 행 이름은 sh_front·sh_34·sh_side 다.

    잡는 회귀: angle 을 그대로 비교하면 17칸 등록의 심사 화면에 사진이 한 장도 안 뜬다 —
