...
def test_cloudflare_exact_url_purge_batches_at_one_hundred(monkeypatch):
    calls = []
    ...
    client = _client()
    client.purge_public_cache([f"users/u/ai/{i}.png" for i in range(205)])

    assert [len(call[1]["json"]["files"]) for call in calls] == [100, 100, 5]
    assert calls[0][0] == "https://api.cloudflare.com/client/v4/zones/zone-1/purge_cache"
    assert calls[0][1]["json"]["files"][0] == "https://images.example.test/users/u/ai/0.png"
    assert calls[-1][1]["json"]["files"][-1] == "https://images.example.test/users/u/ai/204.png"
