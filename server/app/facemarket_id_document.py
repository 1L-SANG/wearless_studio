...
# v1 은 **주민등록증만** 받는다. 마스킹 설계(스펙 §7.2)는 주민등록번호 하나만 고려했고
# 마스크도 사각형 하나뿐이다 — 운전면허증의 면허번호, 여권의 여권번호, 외국인등록증의
# 외국인등록번호는 그 사각형 밖에 있어 **가려지지 않은 채로 최대 7일 저장된다.** 셋 다
# 개인정보보호법 §24 의 고유식별정보이고, 처리방침 §5 는 "수집하지 않는다"고 적혀 있다
# (최종리뷰 I12). 문서 종류별 다중 마스크 영역은 별도 작업이고, 좁히는 건 값싸고 되돌리기
# 쉽다 — 넓힐 땐 마스크 영역과 처리방침 §5 를 함께 고쳐야 한다.
ID_DOCUMENT_TYPES = {"rrc"}
