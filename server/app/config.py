...
class Settings:
    ...
    # 3.7 flash·low 가 prod(3.1 pro·low) 대비 비용 0.47배·지연 -1.3s, 종류 정확도 동률(73%),
    # 성별 18/18(pro 17/18 — 성별 라벨이 있는 표본이 26벌 중 18벌), 특징 2개 온전 25/26(pro 25/26).
    # thinking 승급은 이득 없음(§ 아래). 비용·지연은 **배포되는 프롬프트 기준**이다 — 분류 규칙
    # 블록이 들어가며 프롬프트가 커져 초안 측정치(0.42배)보다 조금 올랐다.
    ...
    # 최종 컷 독립 QC v1.
    # repair = Gemini 1차 → 독립 QC → 필요 시 Gemini 2차 1회 → 독립 재검수.
    ...
    # 정산금을 실제로 보내는 수단. 기본은 manual(관리자가 은행 앱으로 송금하고 기록).
    # `stub` 은 **데모 전용**이고 돈을 옮기지 않는다 — services/payout_provider.py 참고.
    # 모르는 값은 manual 로 떨어진다(오타 하나로 데모 모드가 켜지면 안 된다).
    fm_payout_provider: str = "manual"
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        fm_id_mask_verify=_flag(
            "FM_ID_MASK_VERIFY", "shadow", {"off", "shadow", "enforce"}
        ),
        ...
        fm_payout_provider=os.getenv("FM_PAYOUT_PROVIDER", "manual"),
