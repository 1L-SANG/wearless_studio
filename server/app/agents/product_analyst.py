...
# 그대로(팩트체크 2026-07-13: 국내 커머스 최빈 혼용률, 조사 9종). 유의: ① 패딩·코트 등
# 다층 품목은 '겉감 기준'(상품정보제공고시 관행) ② 실무 표기는 스판덱스↔폴리우레탄 혼용.
...
MATERIAL_PRESETS: dict[tuple[str, str | None], list[dict]] = {
    ...
    ("bottom", "training_pants"): [
        _p("스웻·조거(최빈)", ("면", 70), ("폴리에스터", 30)),
        _p("트랙팬츠·광택", ("폴리에스터", 100)),
        _p("헤비 스웻", ("면", 100)),
        _p("기능성 우위 혼방", ("폴리에스터", 65), ("면", 35)),
    ],
    ("bottom", "jeans"): [
        _p("스판 데님(최빈)", ("면", 98), ("스판덱스", 2)),
        _p("리지드·논스판", ("면", 100)),
        _p("하이스트레치", ("면", 92), ("폴리에스터", 6), ("스판덱스", 2)),
    ],
    ("bottom", "slacks"): [
        _p("TR 슬랙스(최빈)", ("폴리에스터", 70), ("레이온", 25), ("스판덱스", 5)),
        _p("폴리 정장 팬츠", ("폴리에스터", 100)),
        _p("스판 슬랙스", ("폴리에스터", 95), ("스판덱스", 5)),
        _p("울 혼방 프리미엄", ("울", 50), ("폴리에스터", 50)),
    ],
    ("bottom", "skirt"): [
        _p("일반 스커트(최빈)", ("폴리에스터", 100)),
        _p("TR 혼방", ("폴리에스터", 65), ("레이온", 35)),
        _p("코튼 스커트", ("면", 100)),
        _p("스판 H라인", ("폴리에스터", 97), ("스판덱스", 3)),
    ],
    ("outer", "shirt"): [
        _p("코튼 셔켓(최빈)", ("면", 100)),
    ...
    ("outer", "jacket"): [
        _p("일반 재킷(최빈)", ("폴리에스터", 100)),
        _p("TR 블레이저", ("폴리에스터", 70), ("레이온", 25), ("스판덱스", 5)),
        _p("코튼 워크재킷", ("면", 100)),
        _p("나일론 바람막이", ("나일론", 100)),
    ],
    ("outer", "cardigan"): [
        _p("일반 가디건(최빈)", ("아크릴", 100)),
        _p("울 혼방", ("아크릴", 70), ("울", 30)),
        _p("봄 코튼 가디건", ("면", 100)),
    ],
    ("outer", "padding"): [  # 겉감 기준
        _p("폴리 겉감(최빈)", ("폴리에스터", 100)),
        _p("나일론 겉감", ("나일론", 100)),
    ],
    ("outer", "coat"): [
        _p("울 혼방 코트(최빈)", ("폴리에스터", 60), ("울", 40)),
