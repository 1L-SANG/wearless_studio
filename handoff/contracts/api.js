  ...
  function runJob({ duration = 2600, onProgress, result, stall = false }) {
    return new Promise((resolve) => {
      ...
      const id = setInterval(() => {
        ...
        if (stall && pct >= 95 && elapsed < duration + 1200) pct = 95; // 7.4 stall at 95%
  ...
  const api = {
    ...
    async adjustMannequin({ baseId, fit, length, onProgress } = {}) {
      ...
      const next = { ...clone(base), id: base.candidate + '-' + sameCand.length, version: sameCand.length,
        ...
        src: window.Placeholder.photo(base.id + Date.now(), 'mannequin') };
    ...
    async regenerateMannequins({ onProgress } = {}) {
      ...
      ['A', 'B'].forEach((c) => {
        ...
        DB.mannequins.push({ id: c + '-' + n, candidate: c, version: n,
          src: window.Placeholder.photo(c + n + Date.now(), 'mannequin'),
    ...
    async generateImage({ group = '색상 1', onProgress } = {}) {
      ...
      const img = { id: DB.uid('w'), src: window.Placeholder.any('gen' + Date.now()), ai: true };
      (DB.wardrobe[group] = DB.wardrobe[group] || []).push(img);
      return clone(img);
    },
    // a fresh "any image" for the + insert flow
    async pickAnyImage() { await wait(120); return window.Placeholder.any('pick' + Date.now()); },
