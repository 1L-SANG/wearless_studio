...
(function () {
  ...
  /* ---- Catalogs (closed option sets surfaced as chips) ---- */
  const catalogs = {
    clothingTypes: [
    ...
    // measurement schema per clothing type (5.7)
    measurementSchema: {
    ...
    // 단계별 크레딧 단가 — 아직 미확정. 여기 값만 바꾸면 전 화면 예고가 함께 갱신됨.
    creditCosts: {
  ...
  const analysis = {
    ...
    selectedModelId: 'mA', models, matchClothing,
  ...
  /* ---- Mannequin candidates (A/B + history, 7.8) ---- */
  const mannequins = [
  ...
  /* ---- Storyboard blocks (basic mode default, 9.2) ---- */
  ...
  /* ---- Editor blocks: 4-5 prefilled (hook/styling/product) ---- */
  const T = (id, x, y, w, h, text, style) => ({ id: uid('el'), type: 'text', x, y, w, h, text, style: style || {} });
  const IMG = (id, x, y, w, h, src, radius) => ({ id: uid('el'), type: 'image', x, y, w, h, src, radius: radius || 8 });
  const editorBlocks = [
    {
      id: uid('b'), name: '후킹', kind: 'hook', bg: '#ffffff', elements: [
        IMG('i', 60, 50, 880, 560, P.photo('ed_hook', 'horizon', 880, 560), 12),
        T('t', 120, 110, 600, 80, '겨울을 부드럽게, 골지 니트', { size: 40, weight: 600, font: 'Cal Sans', color: '#0e0d14' }),
        T('t', 120, 200, 520, 40, '하루 종일 편안한 데일리 니트', { size: 20, color: '#0e0d14' }),
      ],
    },
    {
      id: uid('b'), name: '셀링포인트', kind: 'selling', bg: '#f5f5f5', elements: [
        IMG('i', 60, 50, 420, 540, P.detail('ed_sell', 420, 540), 12),
        T('t', 540, 150, 380, 40, '부드러운 촉감', { size: 28, weight: 600, font: 'Cal Sans', color: '#0e0d14' }),
        T('t', 540, 210, 380, 80, '코튼 혼방으로 자연스럽게 떨어지는 결, 피부에 닿는 감촉이 부담 없습니다.', { size: 17, color: '#4a4a45' }),
      ],
    },
    {
      id: uid('b'), name: '스타일링컷', kind: 'styling', bg: '#ffffff', elements: [
        IMG('i', 60, 50, 430, 580, P.photo('ed_st1', 'styling', 430, 580), 12),
        IMG('i', 510, 50, 430, 580, P.photo('ed_st2', 'styling', 430, 580), 12),
      ],
    },
    {
      id: uid('b'), name: '호리존컷', kind: 'horizon', bg: '#ffffff', elements: [
        IMG('i', 280, 50, 440, 590, P.photo('ed_hz', 'horizon', 440, 590), 12),
      ],
    },
    {
      id: uid('b'), name: '제품컷', kind: 'product', bg: '#f5f5f5', elements: [
        IMG('i', 90, 60, 380, 500, P.product('ed_p1', 380, 500), 12),
        IMG('i', 530, 60, 380, 500, P.product('ed_p2', 380, 500), 12),
        T('t', 90, 580, 200, 30, 'FRONT', { size: 15, weight: 600, font: 'Roboto Mono', color: '#0e0d14', tracking: 2 }),
        T('t', 530, 580, 200, 30, 'BACK', { size: 15, weight: 600, font: 'Roboto Mono', color: '#0e0d14', tracking: 2 }),
      ],
    },
    /* ---- auto-appended info blocks (PRD §11.22 / §12.8 / §12.9) ---- */
    {
      id: uid('b'), name: '사이즈 안내', kind: 'size', auto: true, bg: '#ffffff', elements: [
        T('t', 60, 56, 500, 44, '사이즈 안내', { size: 28, weight: 600, font: 'Cal Sans', color: '#0e0d14' }),
        T('t', 60, 104, 760, 24, '단위: cm · 측정 위치에 따라 1~3cm 오차가 있을 수 있어요', { size: 14, color: '#4a4a45' }),
        ...product.measurements.slice(0, 4).flatMap((m, i) => {
          const x = 60 + i * 232;
          return [
            T('t', x, 168, 200, 24, m.label, { size: 14, color: '#4a4a45' }),
            T('t', x, 194, 200, 48, (m.value != null ? m.value + ' cm' : '—'), { size: 32, weight: 600, font: 'Cal Sans', color: '#0e0d14' }),
          ];
        }),
      ],
    },
    {
      id: uid('b'), name: '세탁 안내', kind: 'care', auto: true, bg: '#f5f5f5', elements: [
        T('t', 60, 56, 500, 40, '세탁 안내', { size: 24, weight: 600, font: 'Cal Sans', color: '#0e0d14' }),
        T('t', 60, 104, 880, 64, '세탁 전 실제 상품의 케어라벨을 반드시 확인해주세요. 소재와 상품 특성에 따라 관리 방법이 달라질 수 있습니다.', { size: 16, color: '#0e0d14' }),
      ],
    },
    {
      id: uid('b'), name: 'AI 생성 안내', kind: 'ai-notice', auto: true, bg: '#ffffff', elements: [
        T('t', 60, 48, 880, 60, '본 상세페이지의 일부 이미지는 AI를 활용해 생성되었습니다. 실제 상품의 색상과 핏은 촬영 환경 및 화면 설정에 따라 다르게 보일 수 있습니다.', { size: 13, color: '#4a4a45', align: 'center' }),
      ],
    },
  ];
  ...
  window.DB = { account, catalogs, models, matchClothing, product, analysis, mannequins, storyboard, genSteps, editorBlocks, wardrobe, library, uid };
  // expose models inside catalogs too, so the editor's AI panel can default to
  // (and list) the same models picked earlier in the flow (PRD §11.8)
