...
import { prefetchStoryboardAfterProductPhotos } from '../../src/features/storyboard/storyboardEntryPrefetch.js';
...
test('승격은 진행률을 구독자에게 알린다', async () => {
  ...
  const gate = deferred();
  const task = startProductPhotoPromotion('p1', async ({ onPhotoProgress }) => {
  ...
  assert.deepEqual(seen.slice(0, 3), ['pending 0/0', 'pending 0/3', 'pending 2/3']);
...
test('실패도 구독자에게 알리고 프라미스는 거부된다', async () => {
  ...
  const task = startProductPhotoPromotion('p2', async () => { throw new Error('upload down'); });
  const seen = [];
...
test('같은 프로젝트로 두 번 시작해도 업로드는 한 번만 돈다', async () => {
  ...
  let runs = 0;
  const gate = deferred();
  const first = startProductPhotoPromotion('p3', async () => { runs += 1; await gate.promise; return 1; });
  const second = startProductPhotoPromotion('p3', async () => { runs += 1; return 2; });
...
test('프로젝트 id 가 생기면 임시 키의 task 를 그 id 로 옮긴다', async () => {
  ...
  const gate = deferred();
  const task = startProductPhotoPromotion(NEW_PROJECT_KEY, async () => { await gate.promise; return 'x'; });
...
test('콘티보드 진행률 훅은 로딩 early-return 전에 항상 실행된다', () => {
...
test('CTA 는 버려진 임시 키 task 를 지운 뒤 새 승격을 실행할 수 있다', async () => {
  clearProductPhotoPromotionTask(NEW_PROJECT_KEY);
  const stale = startProductPhotoPromotion(NEW_PROJECT_KEY, async () => new Promise(() => {}));
  clearProductPhotoPromotionTask(NEW_PROJECT_KEY);
  let runs = 0;
  const current = startProductPhotoPromotion(NEW_PROJECT_KEY, async () => { runs += 1; });
...
test('프리페치도 사진 정착 뒤에 돈다 — 빈 상품으로 시드된 보드가 캐시되면 안 된다', async () => {
  clearProductPhotoPromotionTask(NEW_PROJECT_KEY);
  const gate = deferred();
  const task = startProductPhotoPromotion(NEW_PROJECT_KEY, async () => gate.promise);
  let prefetched = 0;
  const waiting = prefetchStoryboardAfterProductPhotos('prefetch-project', {
    prefetch: async () => { prefetched += 1; },
  });
  await Promise.resolve();
