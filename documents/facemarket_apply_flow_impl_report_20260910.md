...
## PR 갱신 후 누락된 리뷰 수정 복원

최신 PR 커밋 40b509ea에 기존 검토·검증을 마친 b318ed38의 지원서 수정만 다시 반영했다. 최신 main의 정기결제·로그인 처리, 마이그레이션 번호 20260910180000은 변경하지 않았다.

- 구글·카카오 계정의 연락 이메일 직접 입력·수정과 재지원 연락처 복원.
- 입력한 연락 이메일을 테스트컷 안내에서도 우선 사용.
- 링크 입력 오류의 사전 안내, 제출 오류 원인 구분, 작성 내용·사진·동의 체크 보존 및 수정 후 재시도.
- 속옷·수영복 허용 스위치에 대한 잘못된 FAQ 제거. 선택 목록 제거는 PR 254에서 별도 진행.
- FAQ 기준 문서와 누락된 회귀 테스트 복원. 기존 자체 포함형 FAQ 제목 검사도 유지.

자동 입금 안내 문구에 관한 사용자 결정은 별도 대기이며 이번 복원 범위에서 변경하지 않았다.

복원 검증: 프런트 전체 1,344개, 서버 전체 4,210개 통과(18개 건너뜀), 빌드 통과. 복원 전에는 같은 회귀 검사에서 프런트 8개와 연락 이메일 조회 1개가 실패해 누락을 재현했다.
