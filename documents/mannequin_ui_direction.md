...
3. **순차 확인 스텝 = 핏 · 기장 · … + (상의/아우터면) 매칭 의류 핏.** 스텝마다 **[유지하기] / [조정하기]**(둘 다 중립색). '조정하기'를 누르면 예시 노출(테두리 박스, 참고용) — **넓은 화면(콘텐츠 폭 ≥1100, 컨테이너 쿼리)에선 페이지 오른쪽 고정 패널**: 제목("의류 재현성 높이기") 높이부터 아래로 길게, **2열 그리드**, absolute 라 **가운데 마네킹·아래 확인 카드는 안 움직임**·안 겹침. 좁으면 이미지 옆 인라인 세로 박스(+우측 스크롤바) 폴백. 현재 스텝만 질문을 노출.
4. **매칭 의류 핏은 first-class 순차 스텝(방식 1).** 마네킹컷에 하의가 함께 보이므로 조정 시 **재생성(유료)** 이며 `fitProfile.matchCut`(garment_ref)에 저장된다. 스텝 노출 조건 = **상의/아우터 카테고리 + 매칭 의류를 실제 선택한 프로젝트**(`analysis.matchSelections`에 선택값 존재 — 컷에 하의가 없는데 하의 핏을 묻지 않기 위함). — *구 b2안의 "매칭은 중심 질문 금지·밑줄 링크로만"·"매칭 단독 변경은 무료·saveAnalysis 즉시 저장"은 **폐기**.*
...
6. **전부 확인 → 최종 카드.** 변경 ≥1건 → `수정사항 반영하여 재생성 · N 크레딧`(N = `CREDIT_COSTS.mannequinGenerate` = 2, 백엔드 `credit_cost_mannequin_generate` 미러. 재생성 후 스텝 리셋·재확인 루프). 변경 0건 → `상세페이지 사진 양을 선택해주세요` + **[기본형 / 확장형]** 카드 → `이 사진 양으로 만들기`(`composeMode` store 반영 후 `/create/storyboard` 이동). 두 선택은 핵심 장점 → 핏·코디 → 제품 확인 구조가 같고 사진 수만 다르다.
7. 예시 타일엔 **"선택됨" 상태 금지**(참고용) — "예시" 점선 태그는 **폐기**. 대신 헤더 **"원하는 OO의 예시를 선택해주세요."**(medium) 아래 옵션별 참고 안내 1줄(`fit-ex-sub`, fg-3): 축 스텝 = "예시에 보여지는 의류의 OO만 참고해주세요." / 매칭 스텝 = "예시에 보여지는 하의의 핏만 참고해주세요."
...
- 사진 양 카드 라벨 **15px·600**, **추천 뱃지 없음**. 설명은 간략하게 둔다(mock `composeModes.desc`: 기본형 "대표 컬러 중심으로 필요한 사진만" / 확장형 "같은 순서로 사진을 더 풍부하게").
...
Mannequin
 ...
 │   └─ (changing 시) .fit-ex-col   헤더 + <ExampleTiles> (테두리 박스)
 ...
 └─ .fit-ask (확인 카드)
     ...
     └─ 현재 스텝 질문+[유지/조정]  |  조정 중 바 + [취소]  |  최종(재생성 or 사진 양 선택+만들기)
```
...
- **사진 생성(AG-06)도 fitProfile 소비(완료)**: `cut_generator.render_cut_prompt`가 `analysis.fitProfile`을 동일 카탈로그 블록으로 렌더(FIT PROFILE → PRODUCT CONTEXT 순, 프로필 있으면 레거시 `- Fit:` 생략). 소비처 2곳 모두 배선 — 상세페이지(`detail_page_job`, 기존)와 **에디터 새 이미지 추가(`editor_image_job` mode:'new' — analysis 미로드로 fitProfile이 유실되던 것 수정)** — 선택 마네킹컷 **이미지(1번 참조) + 텍스트 제약 이중 전달**로 순종률 확보(컷 파이프라인 계약). matchCut은 매니페스트에 마네킹 참조나 MATCH 첨부가 있을 때만(없는 하의 지어냄 방지). 확정(무변경) CTA는 이동 전 `saveAnalysis({fitProfile})` **await**(저장→생성 순서 보장). 회귀 = `tests/test_cut_generator.py`.
