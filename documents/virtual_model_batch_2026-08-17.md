...
5. **카탈로그** — `src/features/analysis/AnalysisForm.jsx` AI_MODELS / `src/mock/db.js` AI_MODELS /
   `server/tests/test_frontend_mock_contract.py` expected — 세 곳이 어긋나면 pytest 가 깨진다.
...
- **성별 불균형**: 여성 11 : 남성 3. 성별 칩으로 필터링되므로(`AnalysisForm.jsx:1199`)
...
실행: `node spike/facepack.js --base <베이스컷> --id <sid>v2 --res 4K --identity v5`
