...
- `top.length`: crop=`entire untucked hem at the high waist and visible above any matching bottom`; semi_crop=`entire untucked hem at the natural waist, longer than crop and shorter than basic`; basic=`entire untucked hem at the hip line and not covered by any matching bottom`; semi_long=`entire untucked hem at the low hip, longer than basic and shorter than long`; long=`entire untucked hem below the hips and neither tucked into nor covered by any matching bottom`.
...
### §I 추록 — enforce 즉시 승격 (2026-07-14 사용자 결정)

결정: "미달이면 그 상태로 내보낼 순 없다" — 미달 컷 출고 방지가 오발화 비용(내부 생성 +1콜·지연 수십초)보다 우선.
채택 규칙이 보수적(개선 못하면 원본 유지)이라 하방 리스크 없음이 §I 실증으로 확인된 것이 근거.
- `_MANNEQUIN_AXIS_QC_ENFORCEMENT_READY = True` (리뷰된 커밋 = 의도된 해제 경로), 가드 강등 메커니즘은 테스트로 상시 검증(사고 시 False 롤백 경로 유지)
- `copilot/api/manifest.yml`에 `MANNEQUIN_AXIS_QC: enforce` — main 머지·배포 시 운영 활성
- 골드셋 30쌍 캘리브레이션은 **켠 상태로 병행**(발화율·정확도는 axis_qc/axis_retry 이벤트로 관측), 원 설계의 "게이트 후 승격" 순서를 제품 판단으로 앞당김을 명기
