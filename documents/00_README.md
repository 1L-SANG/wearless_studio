...
- **콘티보드의 사용자 분류는 세 섹션**이다 — 핵심 장점 → 핏·코디 → 제품 확인. `contentRole`은 계속 비노출 자동값이다. 다만 카드 인스펙터에서는 핵심 장점의 스타일링컷·호리존컷, 핏·코디의 스타일링컷·호리존컷·거울샷을 직접 고르고, 제품 확인에서는 고스트샷·디테일샷을 고른다. 생성예시는 현재 컷 종류별로 최대 6장만 보여준다(ADR-0005).
