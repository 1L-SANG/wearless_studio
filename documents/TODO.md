...
- ✅ **실제 생성예시 카탈로그 배선** — 2026-07-20 2단계 완료. 확정 manifest 검증·결정적 WebP thumb·서버 레지스트리 v2·프론트 카탈로그를 함께 만드는 `server/tools/release_genexamples.py`를 추가했고, QC 승인 부분집합을 불변 R2 릴리스 `2026-07-19-pilot-qc-01`로 발행해 서버·프론트에 함께 적용했다. 컷·샷·상품 종류·성별 필터(제품은 성별 공용), 현재 컷 종류 안의 rank 라운드로빈 최대 6장, 발행 variant별 범위 활성화, 같은 공간 pose 게이트, 빈 상태, 에디터/상세페이지 적용성 안전 닫기까지 실제 자산으로 검증했다. 2026-07-21 소비 규칙 개정으로 핏·코디의 서로 다른 컷 종류는 한 갤러리에 섞지 않는다. 디테일은 실제 입력으로 만들 수 있는 핵심 부위만 상품당 1~3개 제공한다.
  ...
  - **서비스용 자산 운영 릴리스 기록(2026-07-20, ADR-0009)** — 해시가 묶인 최종 QC에서 승인된 예시 192개를 발행했다. 원본 variant는 `all` 192·`pose` 12·`bg` 14이고, `all`에서 파생한 WebP thumb 192개를 더해 R2 객체는 410개다. 보류·탈락한 `all` 15개와 미승인 보조 variant는 발행하지 않았다. 이 192개 불변 R2 릴리스는 감사·롤백용 역사로 유지한다.
...
- 🆕 **콘티 시드에 clothingType 배관(백로그, 2026-07-30 오너 결정)** — `src/lib/api/shapes.js defaultStoryboard(colors, mode)`와 mock `src/mock/db.js buildStoryboard(mode, colors)`는 의류 종류를 받지 않아 상의·하의가 같은 컷 구성으로 시드된다. 범위 = 두 빌더에 `clothingType` 전달 → **하의 시드에 뒷면 풀샷 기본 포함**(+ 세트 방향 구성 분기 여지, `createSpaceSetMembers`도 후속). **우선순위는 카탈로그 배선·머지 완료 뒤.**
  - 당장 하의 세트를 상의와 같게 두는 근거(오너 판단): ① 시장조사의 하의 문법 "전신 리드 → 하반신 크롭 → 후면/45도"에서 앞 두 단계가 곧 풀샷+미디움샷이다 — 미디움샷 하의 크롭을 **허리~발**로 확정했으므로(`storyboard_space_set_ui_spec.md` §2, 7/30) "하반신 크롭"이 우리 미디움샷과 일치한다. ② 남는 차이는 뒷면 컷 비중뿐이고 셀러가 방향을 바꾸는 건 클릭 한 번이라 블로커가 아니다.
  - 표본 주의: `cut_sequence_market_research.md`:38의 하의 시퀀스는 드로우핏(스튜디오 카탈로그형) 단일 브랜드 신호가 크다. **시퀀스 전체를 "하의 표준"으로 박제하지 않는다** — 견고한 부분은 "하반신 크롭 + 뒷면 강조"까지다.
