...
- ✅ **contentRole 사용자 노출 제거** — 2026-07-20 완료. 콘티 역할 칩과 역할명을 제거하고 카드는 `이미지 N`으로 표시한다. `contentRole`은 레시피·카피·게이트·정렬용 내부값으로 유지하며, 첫 핵심 장점 AI 카드는 `hero`, 나머지는 기본 구성·섹션·순서·인접 카드와 제품 샷 선택으로 자동 배정한다. 핏·코디는 스타일링·호리존·거울 생성예시를 한 갤러리에서 고르게 하고, 선택한 예시로 내부 역할과 레시피를 바꾼다. 이동·복제·삭제·되돌리기와 서버 저장에서도 다시 정규화한다(ADR-0005 후속 결정).
