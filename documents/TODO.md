...
- ✅ **확장형 다색 컬러웨이 2단 조립** — 추가 색상마다 동일 모델의 서로 다른 포즈로 풀샷(왼쪽)·중간샷(오른쪽)을 한 묶음으로 생성하고, 상품명·상품 색상과 실제 사용한 매칭 의류명·색상을 아래에 표기한다. 대표색은 메인 매칭을 유지하고 추가색은 선택한 메인·서브 중 명도 대비가 큰 한 벌을 묶음 전체에 적용한다(2026-08-13).
...
- ✅ **실제 생성예시 카탈로그 배선** — 2026-07-20 2단계 완료. 확정 manifest 검증·결정적 WebP thumb·서버 레지스트리 v2·프론트 카탈로그를 함께 만드는 `server/tools/release_genexamples.py`를 추가했고, QC 승인 부분집합을 불변 R2 릴리스 `2026-07-19-pilot-qc-01`로 발행해 서버·프론트에 함께 적용했다. 컷·샷·상품 종류·성별 필터(제품은 성별 공용), 현재 컷 종류 안의 rank 라운드로빈 최대 6장, 발행 variant별 범위 활성화, 같은 공간 pose 게이트, 빈 상태, 에디터/상세페이지 적용성 안전 닫기까지 실제 자산으로 검증했다. 2026-07-21 소비 규칙 개정으로 핏·코디의 서로 다른 컷 종류는 한 갤러리에 섞지 않는다. 2026-07-30에는 일반 블록 갤러리에 기존 개별 예시 최대 6장을 먼저 보여준 뒤 같은 조건의 공간 세트 멤버를 추가 선택지로 이어 붙이되, 자동 초안 배정에는 기존 개별 예시만 쓰도록 확정했다. 디테일은 실제 입력으로 만들 수 있는 핵심 부위만 상품당 1~3개 제공한다.
  ...
  - **상의·하의 개별컷 보강 릴리스(2026-08-03)** — 최신 정리 후 활성 58개에 사용자 확정 45개(여성 하의 11·여성 상의 12·남성 하의 11·남성 상의 11)를 더해 새 불변 릴리스 `2026-08-03-individual-styling-02`로 발행했다. 현재 프론트·서버 개별 생성예시는 **103개**이고 R2는 `all` 103·`thumb` 103, 총 **206개**다. 원격 누락·추가·크기 불일치는 모두 0이고 신규 공개 URL 90개도 정상이다. 상의·하의 스타일링 공개 조합 8개를 추가했으며, 제작자 QC HOLD 8개는 사용자 최종 확정을 오너 승인으로 적용하되 원래 판정과 사유를 감사 기록에 보존했다.
  ...
  - **운영 실자산 연결 기록(2026-07-20)** — 당시 R2 410개 키·파일 크기를 업로드 후 전수 대조했고, `server/app/data/example_assets.json`과 `src/data/genExamples.json`을 같은 릴리스로 교체했다. 공개 원본 218개와 thumb 192개 모두 HTTP 200 및 이미지 형식을 확인했다. 이는 2026-07-20 당시 기록이며 현재 활성 레지스트리 수는 위 2026-08-03 상태를 따른다.
...
- 🆕 **마네킹 최초 생성 크레딧 예고** — 마네킹 페이지 진입 시 자동 생성·차감되므로, 분석 CTA 버튼 `의류정보 확정 완료`에 예상 크레딧을 부착(`의류정보 확정 완료 · 45 크레딧`). (`AnalysisForm.jsx:273`) **→ Phase 4**
