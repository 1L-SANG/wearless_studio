...
### 2026-07-31 구세대(파일럿 계열) 소비 제외

- 오너 전수 검토 판정: 구세대 176개 중 **유지 2**(`ex_horizon_women_bottom_medium_01`·`03`), **재생성 앵커 9**(호리존 — 포즈·고개 각도 보존, 우리 배경·하우스 모델로 재생성 예정), **나머지 165 제외**. 앵커 9도 현행 카탈로그에서는 제외(재생성판이 새 ID로 발행되면 복귀).
- 반영: 프론트 `src/data/genExamples.json`·서버 `server/app/data/example_assets.json` 221→**69**(신규 45+유지 2+**제품컷 22 롤백** — 오너 지시, 세트가 못 덮는 유일 영역), 공개 조합표 36→**11**(제품 상의·하의 ghost/detail 재개방).
- **R2 실삭제(오너 지시, 저작권 우려)**: 유지·앵커·제품을 제외한 구세대 159개 ID의 전 변형 **968객체**를 `seed/genexamples/v1/releases/**` 전 릴리스 경로에서 삭제(검증 잔존 0·보존 380). 삭제 키 명단은 `~/Documents/wearless_studio_r2_deleted_keys_2026-07-31.txt`. 릴리스 불변 원칙의 오너 명시 예외로 기록한다.
- 소비 영향: 닫힌 착용 조합의 갤러리는 세트 멤버(setOnly 우회)로 유지, 낱장 자동 배정은 해당 조합에서 미배정(fail-closed). 제품컷은 롤백으로 정상 유지. 기존 프로젝트가 참조하던 구세대 썸네일은 R2 삭제로 깨질 수 있으나 프론트 폴백('썸네일을 불러오지 못했어요' + 다시 선택)이 처리한다.

### 2026-08-01 카탈로그 정합 복구와 제품 고스트 축소

- 구세대 R2 실삭제 뒤에도 삭제된 143개 원본을 가리키던 이전 221개 카탈로그를 폐기하고, R2에 실제 존재하는 자산만 소비하도록 프론트·서버를 함께 정리했다.
- 활성 개별 예시는 새 불변 릴리스 `2026-08-01-active-catalog-01`의 **65개**다: 신규 스타일링 45개, 유지·독립·재생성 호리존 10개, 제품 10개. 승인된 호리존 재생성본 7개를 포함했고, 제외 1개와 보류 1개는 넣지 않았다. 제품 고스트는 오너 결정대로 `ex_product_top_ghost_01` 하나만 유지하고 제품 디테일은 상의 5·하의 4를 유지한다.
- 공개 조합은 **22개**다. 여성 호리존 풀은 전 의류, 남성 호리존 풀은 지원 전 의류, 상단 미디움 호리존은 상의·아우터(여성은 원피스 포함)에 공유한다. 제품 고스트는 상의 조합만 연다.
- R2에는 `all` 65개와 결정적 WebP `thumb` 65개, 총 **130객체**를 발행했다. 봉인 스테이징과 원격 key·파일 크기 전수 대조에서 누락·추가·불일치가 모두 0이다.
- **R2 실삭제(오너 지시)**: 제외한 제품 고스트 12개와 호리존 앵커 `ex_horizon_men_bottom_full_02`의 과거 릴리스 원본·썸네일 **78객체**를 삭제하고 잔존 0을 확인했다. 삭제 키 명단은 `~/Documents/wearless_studio_r2_deleted_keys_2026-08-01.txt`에 보존한다.
- 승인되지 않은 제품 디테일 통일 후보와 보류 중인 호리존 배경 수정 후보는 이 카탈로그에 포함하지 않는다.
