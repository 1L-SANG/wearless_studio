...
- 반영: 프론트 `src/data/genExamples.json`·서버 `server/app/data/example_assets.json` 221→**47**(신규 45+유지 2), 공개 조합표 36→**7**. R2 자산·과거 manifest는 불변 유지(감사 기록) — R2 삭제 없음.
- 소비 영향: 닫힌 착용 조합의 갤러리는 세트 멤버(setOnly 우회)로 유지, 낱장 자동 배정은 해당 조합에서 미배정(fail-closed). **제품 ghost·detail 조합은 전부 폐쇄** — 제품컷 예시 재발행 전까지 제품 갤러리·자동 배정 공백.
