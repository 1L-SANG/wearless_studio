...
## 적용(apply) 가드 — 2026-08-04

`release_genexamples.py`의 `--apply`는 다음을 강제한다. 공간 세트 도구
(`release_space_sets.py`)와 같은 규율이며, 두 도구의 적용 경로를 한 계약으로 맞춘 것이다.

1. **순서 강제** — `--apply`는 **같은 실행의 `--upload --execute`가 성공한 뒤에만**
   쓸 수 있다. dry-run(`--upload`만)으로는 부족하다. 사진을 R2에 올리지 않은 채
   명단만 교체하면 명단에는 있는데 실물이 없는 항목이 생기고, 화면에는 깨진 썸네일로
   남는다 — 2026-08-02에 제품 고스트컷 12장에서 실제로 발생한 사고다.

   강제 지점은 **CLI 인자 검사가 아니라 함수 시그니처**다. `upload_release(execute=True)`
   만 `UploadReceipt`를 돌려주고, `apply_release(result, receipt)`는 그것을 **필수 인자**로
   받는다. CLI 게이트만 두면 `apply_release()`를 파이썬에서 직접 부르는 경로(스크립트·
   다른 도구·향후 호출부)가 그대로 뚫린다. 영수증은 또한
   - 릴리스 ID가 스테이징과 같은지,
   - 스테이징한 자산이 모두 업로드됐는지,
   - **명단이 이번에 올리지 않은 이 릴리스 키를 참조하지 않는지**
   를 함께 검사한다. 마지막 항목이 2026-08-02 사고의 정확한 형태다.

2. **적용 전 전량 검증(fail-closed)** — 덮어쓰기 **전에** 신규 두 문서와 기존 배포본
   두 문서를 모두 검증한다. 하나라도 계약과 다르면 **아무 파일도 건드리지 않고** 중단한다.
   반쯤 적용된 상태가 어느 한쪽이 낡은 상태보다 위험하기 때문이다.

3. **두 파일 교차 검증** — 프론트 카탈로그(`genExamples.json`, 평면 리스트)와 서버
   레지스트리(`example_assets.json`, `_meta`+`assets`)는 서로를 검증한다.
   - 카탈로그 각 항목의 **필드 집합이 정본 스키마와 정확히 일치**해야 한다. id·thumb 만
     보는 얕은 검사로 두면 프론트가 읽는 분류 축(cutType·shot·clothingType·rank·variants…)이
     통째로 빠진 문서도 통과해, 화면에서만 조용히 비는 조합이 생긴다
   - 레지스트리의 모든 자산은 최소 `all`·`thumb`을 갖춰야 한다(`pose`·`bg`는 선택)
   - 두 파일의 예시 ID 집합이 정확히 같아야 한다(한쪽만 갱신되는 사고 차단)
   - 카탈로그의 `thumb`은 레지스트리 `_meta.defaultBaseUrl` 아래여야 한다
   - 같은 ID의 `thumb` URL이 두 파일에서 바이트 수준으로 같아야 한다
   - 카탈로그 ID에 중복이 없어야 한다

4. **롤백** — 카탈로그를 먼저 교체하고 레지스트리 교체가 실패하면 카탈로그를 원래
   내용으로 되돌린다. 두 파일이 어긋난 채 남는 창을 없앤다.

카탈로그가 `_meta` 없는 평면 리스트인 **비대칭은 의도**다. 프론트는 목록만 소비하고,
릴리스 메타데이터의 단일 정본은 레지스트리다. 그래서 형식 검증도 두 갈래로 나뉜다.

### 2026-08-13 제품 디테일컷 통합

- 릴리스 ID: `2026-08-13-product-detail-unified-01`
- 기존 활성 103개 중 구형 제품 디테일 9개를 소비 명단에서 내리고, 사용자가 확정한 회색 의류 디테일 12개(아우터 6·카라티 2·하의 4)를 새 안정 ID로 등록해 프론트·서버 카탈로그를 **106개**로 함께 교체했다.
- 카라티는 잠근 상태와 **위쪽 단추 2개를 푼 상태**를 각각 독립 예시로 발행했다. 아우터 6개는 기존 상의 분류에서 `outer`로 바로잡고 `product/detail/outer/null` 공개 조합을 추가했다.
- 새 불변 R2 prefix에 `all` 106·파생 `thumb` 106, 총 **212개**를 업로드했다. 원격 key·파일 크기 전수 대조는 누락 0·추가 0·불일치 0이고, 신규 원본·썸네일 공개 URL 24개도 모두 HTTP 200이다.
- 적용 중 기존 카탈로그의 nullable `direction`을 릴리스 검증기가 문자열로만 강제하던 모순을 발견해 계약대로 `null` 또는 허용 방향값을 받도록 수정하고 회귀 테스트를 추가했다.

### 2026-09-22 호리존 방향 6컷 재발행(전신 12 + 여성 미디엄 6)

- 릴리스 ID: `2026-09-22-horizon-rotation-18cuts-01`
- 2026-09-21 `2026-09-21-horizon-rotation-6cuts-01`(전신 12장)은 방향 검증은 통과했지만
  기존 라이브러리(`ss_horizon_sequence_05/07`)와 배경·조명·의상 분위기가 갈렸다. 오너 판정으로
  같은 베이스에서 다시 뽑아 **id `_02`로 새로 등록하고, `_01` 12개는 소비 명단에서 내렸다**
  (자산 불변 — R2 원본은 기록으로 남고 활성 URL이 참조하지 않는다).
- 새 사진은 베이스 배경값을 그대로 물려받았다: 남 238/236/234(베이스 237/234/232),
  여 173/169/165(베이스 175/170/164). 성별 안에서 6장 인물·의상·머리모양 동일.
- 여성 미디엄 6컷(`horizon:women:top:medium`)을 새로 채웠다. 이 그룹은 사선 0장이었고
  옆·뒤는 공간 세트 멤버에만 있어 아우터·원피스 조합에서 정면만 보였다.
- 방향 검수는 원본 머리 2~3배 확대로 18장 전수 확인: 정면 두 눈 동일·코 중앙, 사선 두 눈
  노출(먼 쪽 눈 미가림), 90도 눈·귀 각 하나, 뒷면 얼굴 요소 0. 전신 12장은 머리끝~신발 전체 프레임.
- 새 불변 prefix에 `all` 18·파생 `thumb` 18, 총 **36개** 업로드. 신규 공개 URL 36개 모두 HTTP 200.

### 2026-09-22 호리존 미디움 통일 발행 + 남성 미디움 개방

- 릴리스 ID: `2026-09-22-horizon-medium-unified-01`
- 남성 미디움(`horizon:men:top:medium`)은 낱개 예시가 0장이라 갤러리가 공간 묶음 멤버로만 채워졌고
  **사선이 한 장도 없었다.** 기준 촬영(seq07)의 인물·배경으로 6컷(정면·사선 좌우·옆모습 좌우·뒷면)을
  발행하고 `horizon/medium/top|outer/men` 공개 조합을 함께 열었다.
- 여성 하의 미디움(다리 크롭)에 사선·옆모습·뒷면 3컷을 더하고, 배경이 튀던 기존 2장과 새틴 원피스
  1장은 같은 기준으로 다시 뽑아 `_v2` 로 등록했다. 옛 3장은 소비 명단에서 내렸다(자산은 불변).
- 배경 실측은 기준과 채널별 최대 16 이내다(남 238/236/234, 여 174/170/166). 방향은 원본 머리
  2~3배 확대로 12장 전수 확인했다.
- `all` 12·파생 `thumb` 12, 총 **24개**를 새 불변 prefix 에 업로드했고 공개 URL 24개 모두 HTTP 200이다.
- 남은 빈칸: 여성 하의 미디움 계열은 방향별 1장씩이고, 갤러리에 남은 공간 묶음 사진 40장(9세트)은
  아직 옛 촬영이라 배경·조명이 기준과 다르다. 교체본은 만들어 뒀고 세트 릴리스는 별도 건으로 남긴다.

### 2026-09-23 호리존 사선 8장 교체(머리 돌림 재작업)

- 릴리스 ID: `2026-09-23-horizon-3q-headturn-01`
- 사선(`direction: side`, `sideStyle: threeQuarter`) 8장 — 남·녀 × 전신·미디움 × 좌·우 — 을 머리까지
  돌린 새 사진으로 바꿨다. 새 id 는 `ex_horizon_{men|women}_top_full_side[_right]_03`,
  `ex_horizon_{men|women}_top_medium_side[_right]_02` 이고 옛 id(전신 `_02`, 미디움 `_01`)의
  메타(그룹·rank 2/3·적용 의류 종류)를 그대로 물려받았다. **옛 8개는 소비 명단에서 내렸다**
  (자산 불변 — R2 원본은 기록으로 남고 활성 URL이 참조하지 않는다). 여성 하의 미디움 사선은 그대로다.
- 머리 yaw 실측 0.397~0.480으로 8장 전수 통과(오너 확인). 1024×1536 PNG.
- 새 불변 prefix에 `all` 8·파생 `thumb` 8, 총 **16개** 업로드. 신규 공개 URL 16개 모두 HTTP 200.

## 7. 갤러리 나열 규칙 (2026-08-05 오너 확정)

새 생성예시를 추가해도 아래 규칙에 맞으면 갤러리 배치가 자동으로 정리된다. 규칙의 코드 정본은
`src/lib/exampleMoodOrder.js`(버킷·키워드)와 `src/lib/generationExamples.js`(조립 순서)이며,
콘티보드 인스펙터와 에디터 새 컷 패널이 같은 컴포넌트(MoodGuide)를 쓰므로 **두 화면에 동일 적용**된다.

### 나열 순서
1. **일반 예시** — 장소 버킷 순: 카페 → 집·실내 → 거리·도심 → 공원·자연 → 해변·리조트 → 전통·헤리티지 → 기타.
   버킷 안에서는 `rank` 오름차순, 동률이면 `id` 사전순(안정 정렬).
2. **세트 유래 예시**(setOnly) — 일반 예시 뒤에 붙는다.
3. **거울 예시**(cutType: mirror) — 스타일링 갤러리의 **맨 뒤** 클러스터. 컷 종류 탭에는 거울이 없고
   (UI에서만 제외 — 저장·이동 규칙에는 존치, `storyboardTaxonomy.js` 참조), 샷 탭과 무관하게 항상 노출,
   구분 라벨 없음. 선택하면 그 컷이 거울컷 레시피로 전환된다(예시 선택이 샷·컷종류를 바꾸지 않는다는
   원칙의 **의도적 예외**).

### 버킷 판정
`mood` · `detailSubject` · `id` 문자열에 버킷 키워드가 포함되는지로 판정한다(자유 문장 허용).
여러 버킷에 걸리면 좁은 장소 버킷(전통 → 해변 → 공원 → 카페 → 집)을 먼저 판정하고,
넓은 거리·도심 버킷을 나중에 판정한다. 이 판정 우선순위는 위의 표시 순서와 별개다.
무매칭은 '기타'로 가서 맨 뒤에 나열된다
— 동작은 하지만 발견성이 나빠지므로 아래 체크리스트를 따를 것.

### 새 예시 추가 체크리스트
1. `mood` 문장에 장소가 드러나는 키워드를 넣는다(cafe/home/street/park/beach/heritage 계열 —
   전체 어휘는 `exampleMoodOrder.js`의 `EXAMPLE_MOOD_BUCKETS`).
2. 새로운 장소 유형이면 해당 버킷의 keywords에 단어를 추가한다(새 버킷 신설은 오너 확정 필요).
3. `node --test tests/frontend/example-mood-order.test.mjs` 로 확인한다 — 실데이터 전수를 돌려
   나열을 검증하며, '기타' 비율 상한 가드가 있어 어휘 누락이 커지면 테스트가 먼저 알려준다.
4. 거울 예시는 `cutType: 'mirror'` 만 지정하면 위 규칙대로 자동 배치된다(탭·라벨 작업 불필요).
5. `night`·`travel`·`outdoor`는 현 데이터가 의존하는 비장소 토큰이므로, 추가·변경 시 좁은 장소 키워드를 가리지 않는지 실데이터 회귀를 확인한다.
