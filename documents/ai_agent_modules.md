...
| `detail_cut` | **GPT Image 2 snapshot** (`gpt-image-2-2026-04-21`) | PL-4 콘티 일반 컷의 Stage 1·2. 그중 확정 대상은 별도 exact 입력·프롬프트 패킷을 사용. 프로덕션은 명시 설정 |
...
| AG-06 | cut-generator (사진 생성 — 내부 mirror 레시피 포함) | detail_cut(PL-4 일반 컷) / image_signature(첫 시그니처) / image_high(에디터) | `generateDetailPage`(컷 단계), `generateImage(mode:'new')` | storyboardPerCut / editorImage | ✅ (백엔드 라이브 — detail_page_job·editor_image_job) |
...
| AG-P2 | independent cut/page-qc (이미지·페이지 검수) | text | AG-06 컷 직후 + 상세페이지 컷 묶음 완료 후 | — | ✅ shadow 라이브(기본 off, 비차단) |
...
| 출력 | 분석 raw: `{ clothingType, subCategory, customCategory(자유 한국어, enum 밖 의류 명칭), targetGenders, fit, materials[], aiSuggestedPoints(≤2), suggestedName, swatchSuggestions: { colorGroupId, swatchId }[], styleTags: string[], confirmedGptProductEvidence }` 구조화 JSON 강제. `confirmedGptProductEvidence`는 패널별 판독 가능성, hard facts, uncertainties와 `hem_shape`, `cuff`, `button_count_visible`, `pattern_structure`, `surface_texture`, `seam_lines` 여섯 고정 사진 관찰을 서버 소유 입력 바인딩과 결합해 analysis payload에만 저장하고 일반 클라이언트 결과에서는 숨긴다. visibleSurfacePlan은 서버 라우팅 정책만 담는다. 이 raw는 그대로 저장되는 게 아니라 서버가 아래 규칙으로 분배한다. `aiSuggestedPoints`는 **AG-08(전용 특징 에이전트, 병렬)** 결과가 있으면 그것으로 교체하고 AG-01 값은 폴백으로 쓴다. `materials`가 비면 서버가 카테고리 보편 조성(DEFAULT_MATERIALS, 팩트체크 2026-07-13)으로 채운다. |
...
### AG-06 cut-generator — 이미지 생성 (상위 계열: styling·horizon·product)

`cutType`은 페이지를 나누는 콘티 분류가 아니라 이미지 생성 레시피 입력이다. 실행 전 `styling|horizon|product` 세 계열로 컴파일하며, 기존 저장값 `mirror`는 `styling/mirrorSelfie` 하위 방식으로 읽는다. 콘티 카드 인스펙터에서 사용자가 섹션별 허용 컷 종류를 직접 고르면, 앞 단계가 그 값을 저장하고 내부 `contentRole`을 자동으로 맞춘다. AG-06은 선택된 계열을 렌더링하며, 생성예시는 별도 `all|pose|bg` 범위 안에서 가능한 구도와 분위기를 보탠다(ADR-0005·0010).

| | |
|---|---|
| tier | PL-4 일반 컷은 `detail_cut` GPT를 쓰고, 첫 시그니처 컷은 별도 `image_signature` 계약을 쓴다. PL-5 에디터만 기존 `image_high` 계약을 유지한다. exact 여부는 모델 선택이 아니라 과거 입력·프롬프트 패킷 적용 여부를 구분한다. |
...
| exact 이미지 입력 순서 | 선택 resolved 마네킹 → `grid_face_direction` → `grid_fullbody` → 셀러 원본 1~4장을 합친 라벨 근거 그리드 **한 장** → 선택 매칭 최대 한 장 → 서비스 생성예시. 방향 시트 두 장은 한 원자 쌍이다. SHA/길이로 검증한 각 입력은 provider multipart에도 그 바이트와 MIME 그대로 전송하며 PNG 재인코딩을 거치지 않는다. |
...
> **구현 구조 (2026-06-20 결정 → 2026-08 개정)**: generic 프롬프트는 **단일 섹션 템플릿** `server/prompts/cut_generate_v1.txt`의 하위 실행 섹션으로 통합 — 구 `prompts/cuts/*` 컷별 파일은 삭제됐다. `server/app/agents/cut_plan.py`가 저장 호환용 mirror까지 읽어 세 상위 계열과 항목별 정본을 먼저 확정하고, `cut_generator.py`는 활성 하위 섹션만 렌더한다. PL-4의 일반 컷은 모두 `detail_cut` GPT를 사용하되, 확정 조건을 모두 만족한 블록만 별도의 hash-pinned exact 프롬프트·패킷을 쓴다. 첫 시그니처 컷은 `image_signature`, 에디터 새 이미지·마네킹·매칭·AG-07은 공유 `image_high`의 기존 경로를 유지한다. `styling` = 일상/룩북 컷(별도 '일상' cutType 신설 안 함 — 라벨만).
...
> **현재 배선과 연구 경계 (2026-08-18)**: 확정 GPT exact 경로는 리사이즈·재인코딩하지 않은 원본 바이트/MIME의 `grid_face_direction + grid_fullbody`를 원자 쌍으로 사용하고, provider에도 동일 바이트/MIME를 보낸다. 그 밖의 generic VIRTUAL 착용컷은 기존 `face_front + body_front` 계약과 OpenAI 호환 PNG 정규화를 유지한다. exact 대상으로 들어온 뒤 자산이 없다고 generic 쌍으로 바꾸지 않는다. 아래 `C방식 3장`은 2026-08-04 연구 기록이며 현재 exact 입력 계약이 아니다.
>
...
- **AG-P2 independent cut/page-qc** (`text`, 비전 입력): 현재 AG-06의 상세페이지 컷과 에디터 `mode:'new'` 결과를 독립적으로 판정한다. 컷 판정은 컴파일된 CutPlan과 역할이 붙은 정본 이미지, 후보만 받고 제작 프롬프트·제작자 판정은 받지 않는다. 상품 동일성에는 색·구조·소재·무늬·부자재뿐 아니라 상품의 텍스트·로고도 포함한다. 페이지 판정은 같은 SKU·목표 색상·모델·매칭·아우터 이너와 같은 `spaceGroupId` 안의 공간 연속성을 본다. `CUT_OUTPUT_QC_MODE`는 `off|shadow|repair`, `PAGE_OUTPUT_QC_MODE`는 `off|shadow`다. PL-4 exact 경로의 `repair`는 `GPT Stage 1 → 독립 QC → 조건부 Stage 2 → 독립 재검수`다. 생성예시의 실제 프레임 범위·머리 경계를 어긴 `framingCrop`, `anatomyPerspectiveAsymmetry`, `lightingShadowReflectionDrape` 실패만 Stage 1을 같은 GPT로 직접 편집한다. 미디움→풀샷이나 의도된 머리 크롭 소실은 기존 모델·의류·공간을 고정한 확대·크롭으로 다룬다. 방향·포즈·시선·카메라 의미를 포함한 `framingDirectionFacePose`와 그 밖의 실패는 같은 exact 입력 패킷과 고정 프롬프트에 교정 사실을 더해 처음부터 같은 GPT로 재생성한다. Stage 2는 정확히 한 번만 시도하고 독립 재검수상 개선된 경우에만 교체한다. 프로덕션 PL-4 일반 컷의 1·2차는 `detail_cut=gpt-image-2-2026-04-21`, `DETAIL_CUT_IMAGE_SIZE=4K`이며 2:3 요청은 `2336x3504`로 변환한다. exact 대상만 과거 패킷을 추가 적용한다. 첫 시그니처 컷은 별도 `image_signature`, 에디터 PL-5와 AG-07은 공유 `image_high` Gemini를 유지하며, PL-5는 전역 값이 `repair`여도 shadow 관측만 한다. exact 입력 준비 단계의 누락·불일치는 fail-closed지만, 성공한 Stage 1 뒤 `UNJUDGEABLE`·QC 불능·Stage 2 실패·회귀는 추가 호출 없이 Stage 1을 보존한다. 강제 HOLD는 별도 보정 뒤의 enforce 단계다(ADR-0010).
  - **선례(메커니즘만)**: 스파이크(`spike/codex-phase4-mannequin-job-design.md` §5)에 **비-AI 싼 QC**(Pillow 크롭/프레이밍/고스트 휴리스틱)를 1차 게이트로 두고, `format_qc_feedback()`이 실패 사유를 다음 시도 프롬프트에 붙이는 **피드백 재시도 루프**가 설계돼 있다. AG-P2는 이 루프의 **의미(semantic) 단계**를 채운다(동일한 correctionPrompt 주입 메커니즘을 '같은 옷인가' 판정으로 확장).
    - ⚠️ **폐기 주의**: 스파이크의 *Flash 기본 → QC 실패 시 Pro 승격(4회 escalation)* tier 설계는 **현행 §1 라우팅에서 폐기**. PL-4 일반 컷은 `detail_cut` GPT를 쓰며 exact 대상만 과거 패킷을 추가 적용한다. 첫 시그니처는 `image_signature`, 마네킹·매칭·에디터·AG-07은 각 기존 공유 `image_high` Gemini 경로를 직접 쓴다. `image_light`(Flash)는 MVP 미배정이며 별도 Flash→Pro 승격 단계는 없다.
