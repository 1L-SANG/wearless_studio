...
2. 멤버 `order`는 1부터 연속·유일하고 manifest 배열 순서와 같다.
   `horizon-sequence`는 2~7컷, `styling`은 2~5컷이며,
   `horizon-rotation`은 기존대로 정면·옆면·뒷면 전신 정확히 3컷이다.
   호리존은 완성 `all`이 필수이고 `pose`는 생략/null을 허용한다.
   스타일링은 `all`과 `pose` 모두 필수다. 기존 실제 포즈 자산은 유지하며,
   새 `all` 사진을 포즈 자산으로 위장하지 않는다. 프론트 멤버의 `variants`는
   실제 발행 상태(`['all']` 또는 `['all','pose']`)를 기록한다.
   구형 프론트 카탈로그의 `variants` 생략은 기존 두 자산 발행 계약으로 읽는다.
   호리존 그룹은 `horizonReference`로 선택한 완성 예시의 원래 샷·방향을 전달하며,
   현재 컷의 샷·방향 변경은 허용한다. 스타일링의 `poseReference`는 유지한다.
   분리된 컷이 `pose` 범위를 명시하면 실제 포즈 자산이 있어야 한다.
...
8. 업로드는 기본 dry-run이다. 첫 실행은 새 스테이징에 대해 `--upload`로
   ...
   실행한다. 이 순서는 CLI 밖에서도 우회할 수 없다. 실제 업로드만
   `UploadReceipt`를 반환하고 `apply_release(result, receipt)`가 이를 필수로
   받아, 이번 릴리스의 전체 자산과 명단이 참조하는 키를 모두 확인한다.
