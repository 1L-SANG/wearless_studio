...
> 2026-09-08 마네킹 변경: 마네킹의 최초 생성과 사진 보완 재시도는 `photo_evidence` 정책을 사용한다. 이 경로에서는 혼용률과 아래의 혼용률 기반 질감 설명을 이미지 요청에서 제외한다. 저장된 소재 정보와 다른 경로의 `legacy` 정책은 유지한다. 현재 마네킹 요구사항은 `documents/PRD.md`의 7.3.1을 따른다. 아래 내용은 소재 블록의 이력과 남아 있는 참조 구현을 설명한다.
