...
> **2026-07-04 스코프 축소 (임베딩 0)**: 아래 스키마 중 **벡터 부분은 전부 보류** — `text_embedding`/`image_embedding` 컬럼, `matching_embeddings`·`ref_images` 테이블, `create extension vector`. **지금 적용할 결정적 테이블은 2개뿐**: `style_affinity`(태그 친화도 맵) + `kb_chunks`(단, `text_embedding` 컬럼 제외 — `keys`+`body_en`만). 셀러 정규화(FR-D1)는 alias 사전이라 테이블 불요(코드 상수/시드). 커밋된 마이그레이션 `20260704000000_retrieval_pgvector.sql`은 벡터 포함 원본 — 적용 전 축소본으로 대체 필요(후속).
...
| R3 | ~~Vertex 인증~~ **소멸** (D2 자체호스팅 확정 2026-07-04). 신규: torch ~2GB 의존이 Railway 이미지·콜드스타트 증가 | 임베딩을 배치/워커 프로세스로 분리(API 비대화 방지), 모델 가중치 1회 캐시. CPU 배치라 GPU 불요. 1b·3의 Vertex 차단 원인 제거됨 |
