...
- **S3 (레퍼런스)**: 스타일링컷 생성 시 같은 무드의 레퍼런스 이미지가 `refImages`로 들어가 4컷의 톤·무드가 한 페이지처럼 읽힌다.
...
| FR-A4 | **v2 = 벡터 챌린저.** 이미지 임베딩 기반 조화 랭킹을 같은 flag 체계 뒤에 두고, 평가(§7)에서 v1을 **이길 때만** 승격 | P1 |
| FR-A5 | 호출부는 **동기 GET 라우트**(`routes.py:484`)다. v2의 쿼리 임베딩은 요청 중 계산 금지 — 분석(PL-1) 시점에 미리 계산해 저장하거나, 사전 임베딩된 값만 사용. GET p95 예산 +50ms | P1 |
...
| FR-B4 | **v2 = kb 벡터 검색 챌린저.** `kb_chunks` 텍스트 임베딩 검색이 v1 정적 선택을 평가에서 이길 때만 승격 | P1 |
...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-D1 | **셀러 텍스트 정규화 검색(canonicalization).** 대상: **강조특징(sellingPoints·aiSuggestedPoints)만** — 자유 입력 칩 경로(`AnalysisForm.jsx:214` → `prompts.py:68`). **상품명·소재명 제외**(상품명은 리터럴 정체성이라 치환 부적합 — `prompts.py:76`; 소재명은 `materials.py`가 이미 canonical 처리 — 이중 정규화 금지). 방식: **운영자 큐레이션 canonical 표현 어휘**(영문 본문, `kb_chunks kind='selling_point'`)에서 텍스트 임베딩 최근접 매핑. 유사도 임계 미달 시: shadow=현행 sanitize 원문 유지+매핑 로그, enforce=미달 항목 제외. flag `SELLER_TEXT_CANONICALIZE=off\|shadow\|enforce` | P0 |
...
| FR-D2 | **Pillow QC 게이팅 활성화.** ✅ **완료 (2026-07-04)** — `scripts/qc_calibrate.py`로 캘리브레이션(FP 0/15, 실패모드 검출 20/25 — 놓침 5건은 전부 crop_zoom, 픽셀 기하로 원리적 구분 불가라 AG-P2 몫으로 문서화). 유령 판정을 2-레짐으로 개선(`qc.py` — 흰옷·모노톤 정상을 차단하던 잠재 버그 수정), 회귀 테스트 7종 추가(`tests/test_qc.py`), `MANNEQUIN_QC_ENABLED=true` (.env·.env.example). **Railway 프로덕션 env에 동일 키 수동 등록 필요** | ✅ |
...
```
   ...
   scripts/embed_corpus.py┘  embed_text=OpenAI / embed_image=Vertex(→R3 폴백)
        요청 경로:
...
| 파일 | 역할 |
|---|---|
| `server/app/services/embeddings.py` | `embed_text()`(OpenAI)/`embed_image()`(Vertex). 모델은 `model_routing.py` 패턴 단일소스. 관측 로깅 |
...
| R3 | Vertex 멀티모달 임베딩 인증(GCP 서비스계정/ADC — AI Studio 키 불가) | 1b·3 전 프로비저닝. 폴백: self-host SigLIP/open-CLIP 또는 타 벤더. **1a·2a·2b는 무영향** |
...
| # | 이슈 | 결정 |
|---|---|---|
| 1 | Vertex 인증 경로 | ✅ **GCP 프로젝트 셋업** — 서비스계정 발급 → Railway env 등록. 1b 착수 전 프로비저닝 |
