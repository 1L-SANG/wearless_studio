...
### 7.1 `uploadAsset` — 'AI 분석하기' 시점 업로드

분석은 이미지가 R2에 있어야 가능하다. **업로드는 제출('AI 분석하기') 시점에 일괄 수행한다** (사용자 결정 2026-07-03 — 초기 구현은 추가 시점 업로드였으나, 사진 추가는 원래 mock처럼 즉시·로컬이어야 하고 검사/실패는 명시적 액션 버튼에서 나는 게 자연스럽다는 피드백으로 변경). 규칙:
- 추가 시점: objectURL 미리보기 + `file` 원본 보존만. 서버 호출·실패 토스트 없음.
- 제출 시점: `file` 보유 이미지만 업로드 → asset id/URL로 치환. **부분 성공 보존**(중간 실패 시 성공분은 상태에 반영 — 재시도 시 중복 업로드 방지), 실패는 **서버의 구체 사유를 살려** `'{파일명}' 업로드에 실패했어요 — {사유}` 형태로 입력 단계 롤백+토스트.
- 서버 실패 문구도 구체화: 형식 거부 = "지원하지 않는 사진 형식이에요. JPG·PNG·WebP로 올려주세요. (아이폰 HEIC 사진은 JPG로 변환 필요)" · 크기 = "사진이 너무 커요. 15MB 이하 사진으로 올려주세요." (실사용 첫 브라우저 테스트에서 HEIC 400의 사유가 삼켜진 것 반영)
...
| 14 | 업로드 실패 (형식·크기·네트워크) | 프론트 (submit ①) | 입력 단계 롤백 — 성공분은 보존(재시도 시 그 사진만 재업로드) | "'{파일명}' 업로드에 실패했어요 — 지원하지 않는 사진 형식이에요. JPG·PNG·WebP로 올려주세요. …" (서버 구체 사유 그대로) |
...
8. ~~업로드 중 타일 표시~~ — **해소(2026-07-03)**: 업로드가 제출 시점으로 이동해(§7.1) 추가 시점 네트워크 대기가 사라졌고, 제출 중 대기는 기존 분석 스켈레톤이 커버한다.
