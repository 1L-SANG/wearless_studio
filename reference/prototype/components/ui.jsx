...
function Button({ children, variant = 'ghost', size, block, onClick, disabled, icon, iconRight, type, style }) {
  return (
...
function IconButton({ name, onClick, active, size = 'md', title, stroke }) {
  return (
...
function Field({ label, opt, hint, ...props }) {
  return (
...
function Chips({ options, value, onChange, multi, star, className }) {
...
function TagInput({ tags, onChange, max = 5, placeholder = '내용을 입력하고 Enter' }) {
...
function ProgressBar({ value, label, sub }) {
  return (
    <div>
      {(label || value != null) && (
...
function ErrorState({ title = '문제가 발생했어요', desc, onRetry }) {
  return (
...
function ThumbGrid({ items, value, onChange, cols = 4, labels }) {
  return (
...
Object.assign(window, {
  Icon, Button, IconButton, Field, Chips, TagInput, Toggle, ProgressBar,
  Checklist, Modal, EmptyState, ErrorState, Skeleton, ThumbGrid,
