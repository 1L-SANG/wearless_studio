/* =============================================================
   features/storyboard.jsx — ⑤ 콘티보드 (PRD §9)
   Left: block cards (select to edit; selection locks others until 수정완료).
   Right: inspector (props) → slide-up pose/bg detail editor.
   Bottom: fixed action bar (이전 · 카피라이팅 토글 · 컷 요약 · 생성).
   확정 레이아웃: 좌측 블록 카드 + 우측 인스펙터(split).
   ============================================================= */
...
const hexFor = (c) => COLOR_HEX[c.swatchId] || COLOR_HEX[c.name] || '#d8d6dc';
...
function UnderlineTabs({ options, value, onChange }) {
...
function ColorDots({ colorOpts, value, onChange }) {
  return (
...
/* mood-guide grid: 1-row horizontal scroll.
   ...
   cut setup (PRD §9.9 — 생성예시는 방향·샷에 따라 달라진다). */
function MoodGuide({ catalogs, cut, direction, shot }) {
  ...
  const examples = React.useMemo(() => Array.from({ length: 8 }, (_, i) => {
    ...
    return { id: seed, thumb: window.Placeholder.photo(seed, cat, 240, 320) };
...
function Inspector({ block, catalogs, colorOpts, mode, onMode, onChange, matchClothing, dirty, warn, onDone, onRevert, onAddMine, onImgDrag }) {
  ...
  // 내 이미지 = 직접 삽입 흐름 (PRD 9.8) — no AI options
  const isMine = block.source === 'mine';
  if (isMine) {
    return (
    ...
    <div className="wizard wide sb-page">
      ...
      <div className="sb-actionbar">
        <div className="sb-ab-inner">
          <button className="btn btn-ghost" onClick={() => history.length > 1 && (location.hash = 'mannequin')}><Icon name="arrowLeft" size={17} />이전</button>
          ...
          <button className="btn btn-primary btn-lg sb-ab-go" onClick={onNext}>
...
// share the 컷 종류 underline-tab control with the editor's AI panel so both
// surfaces use the SAME control for the same concept (consistency, PRD §9.8/§11.8)
window.UnderlineTabs = UnderlineTabs;
// also share the color-dot picker, mood-example grid, and the color→hex helper
// so the editor's AI "새 컷 추가" matches the storyboard inspector exactly
window.ColorDots = ColorDots;
window.MoodGuide = MoodGuide;
