...
alter table public.fm_biometric_enrollments drop constraint if exists fm_biometric_enrollments_height_bucket_check;
alter table public.fm_biometric_enrollments add constraint fm_biometric_enrollments_height_bucket_check
  check (height_bucket is null or height_bucket in (
    'm_lt170','m_170_175','m_175_180','m_180_185','m_185_190','m_gte190',
    'f_lt155','f_155_160','f_160_165','f_165_170','f_170_175','f_gte175'
  ));

alter table public.fm_biometric_enrollments drop constraint if exists fm_biometric_enrollments_body_type_check;
alter table public.fm_biometric_enrollments add constraint fm_biometric_enrollments_body_type_check
  check (body_type is null or body_type in (
    'delicate','slim','regular','plump','toned','bulk','glamorous'
  ));
