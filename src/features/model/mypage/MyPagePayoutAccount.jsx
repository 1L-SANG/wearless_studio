...
import { getFacemarketConfig } from '@/lib/api/facemarket.js';
import { MyPageDialog } from './MyPageDialog.jsx';
import { PAYOUT_ACCOUNT_API_READY, PAYOUT_BANKS, getPayoutAccount, savePayoutAccount, validatePayoutAccount, normalizePayoutAccount, maskAccountNumber, payoutAccountLabel, payoutBanksFromConfig } from './payoutAccount.js';
...
export function usePayoutAccount(modelId, enabled) {
  ...
  const [banks, setBanks] = useState(PAYOUT_BANKS);
  const [attempt, setAttempt] = useState(0);
  useEffect(() => {
    if (!PAYOUT_ACCOUNT_API_READY || !enabled) return;
    let alive = true;
    setPhase('loading');
    Promise.allSettled([getPayoutAccount(), getFacemarketConfig()]).then(([accountResult, configResult]) => {
      if (!alive) return;
      if (configResult.status === 'fulfilled') setBanks(payoutBanksFromConfig(configResult.value));
      if (accountResult.status === 'fulfilled') {
        setAccount(accountResult.value); setPhase('ready'); setError('');
      } else if (accountResult.reason?.status === 404) {
        setAccount(null); setPhase('ready'); setError('');
      } else if (accountResult.reason?.status === 503 || accountResult.reason?.code === 'payout_account_unconfigured') {
        setAccount(null); setPhase('unavailable'); setError('');
      } else {
        setPhase('error'); setError(accountResult.reason?.message || '입금 계좌를 불러오지 못했어요.');
      }
    });
    return () => { alive = false; };
  }, [modelId, enabled, attempt]);
  return { account, phase, error, banks, setAccount, retry: () => setAttempt(value => value + 1) };
}
...
export function AccountShortcut({ bank, onOpen }) {
  return <button type="button" className={`${s.quietButton} ${s.accountShortcut}`} disabled={!PAYOUT_ACCOUNT_API_READY || bank.phase !== 'ready'} onClick={onOpen}>
    ...
    {bank.phase === 'unavailable' && <span>준비 중</span>}<ChevronRight className={s.icon} aria-hidden="true" />
  </button>;
}
...
export function BankSection({ bank, onOpen }) {
  return <section className={s.bankSection} aria-label="입금 계좌"><div className={s.bankHeading}>
    ...
    <div><h3>입금 계좌</h3><p>{bank.phase === 'unavailable' ? '입금 계좌 등록은 준비 중이에요. 열리면 여기에서 등록할 수 있어요.'
      : bank.phase === 'loading' ? '입금 계좌를 불러오고 있어요.' : bank.phase === 'error' ? bank.error
      : bank.account ? payoutAccountLabel(bank.account, bank.banks) : '정산금을 받을 본인 명의 계좌를 등록해 주세요.'}</p></div>
...
export function PayoutAccountDialog({ account, banks = PAYOUT_BANKS, model, onClose, onSaved }) {
  ...
  const save = async event => {
    ...
    try {
      ...
      onSaved({ bankCode: updated?.bankCode || normalized.bankCode, holderName: updated?.holderName || normalized.holderName,
        bankName: updated?.bankName || banks.find(bank => bank.code === normalized.bankCode)?.name,
        accountMasked: updated?.accountMasked || maskAccountNumber(normalized.accountNumber) });
    ...
    <form onSubmit={save} noValidate><p>정산금을 받을 본인 명의 계좌를 등록해 주세요.</p>
      <fieldset className={s.bankFields} disabled={busy || !PAYOUT_ACCOUNT_API_READY}>
        <label className={s.bankField}>은행<select value={form.bankCode} onChange={event => update('bankCode', event.target.value)}>
          <option value="">은행 선택</option>{banks.map(bank => <option value={bank.code} key={bank.code}>{bank.name}</option>)}</select></label>
        <label className={s.bankField}>계좌번호<input type="text" inputMode="numeric" autoComplete="off" pattern="[0-9-]{8,24}" maxLength={24}
          placeholder={account?.accountMasked || '숫자와 하이픈으로 입력'} value={form.accountNumber} onChange={event => update('accountNumber', event.target.value)} /></label>
