...
export function renderConsent(consents, setConsents, withdrawalOpen, setWithdrawalOpen) {
  const check = (index, label) => <label className={s.consentLabel} htmlFor={`consent-${index}`}><input id={`consent-${index}`} type="checkbox" checked={consents[index]} onChange={(event) => setConsents((current) => current.map((value, i) => i === index ? event.target.checked : value))} /><span>{label}<span className={s.required}> (필수)</span></span></label>;
  return <>
    {heading('먼저 본인인지 확인해요', '실명인증을 먼저 진행하고 본인임을 동의해야 이어나갈 수 있습니다.')}
    ...
    <div className={s.consentList}>
      ...
      <div className={s.consentCard}><span className={s.tag}>안내 · 동의 아님</span><p className={s.consentLabel}><span>얼굴 정보는 착용컷을 만들기 위해 미국의 저장소와 이미지 생성 서비스로 전송돼요. 계약 이행을 위한 위탁이라 동의 대신 알려 드려요(개인정보 보호법 제28조의8 제1항 제3호).</span></p><div className={s.legalSummary}>
        <p><b>받는 곳:</b> Cloudflare, Google, OpenAI(미국)</p><p><b>언제:</b> 사진을 저장할 때, 착용컷을 만들 때</p>
        <p><b>무엇:</b> 사진, 얼굴 특징정보, 얼굴 참조 자산</p><p><b>기간:</b> 저장은 라이선스 유지 동안, 생성은 최대 55일 뒤 삭제</p>
        <p><b>거부:</b> 처리방침 제6조의 연락처로 알릴 수 있지만, 저장과 생성이 국외에서 이뤄져 등록은 진행할 수 없어요.</p>{legalLink('/overseas-transfer', '국외 이전 안내 전문 보기')}
      </div></div>
    </div>
  </>;
}
...
export function renderPhotos({ sub, enrollment, previews, busy, onFile, onRemove, editGroup }) {
  ...
  return <>
    {heading('얼굴 이미지 업로드', 'digital DNA를 제작하기 위해선 여러 장의 사진이 필요해요. 아래 예시 이미지와 같은 사진을 동일하게 업로드해 주세요.')}
...
export function renderConditions({ terms, setTerms, body, setBody, busy, priceAgreed, setPriceAgreed }) {
  ...
  return <>
    ...
    <section className={s.bodySection}>
      <h2>몸의 두께</h2><p className={s.bodyDescription}>셀러가 옷을 고를 때 참고해요. 고르지 않아도 괜찮아요.</p>
