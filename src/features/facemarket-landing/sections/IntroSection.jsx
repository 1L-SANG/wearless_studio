/* 첫 화면 아래 — 모델 관점 리드문 + CTA. 원래 히어로 제목 바로 밑에 있던 것을 캐러셀
   아래로 내렸다(HeroSection.jsx 머리말 참고). 셀러·브랜드 관점 문구는 여기 들어오지 않는다. */
import { Icon } from '@/components/ui.jsx';
import s from '../FacemarketLanding.module.css';

export function IntroSection({ onPrimary, primaryLabel }) {
  return (
    <section aria-label="소개" className={s.intro}>
      {/* 리드는 "절차 한 줄 + 편익 한 줄"이다. 아래 두 문장은 라운드마다 반대로
          왕복한 자리라 근거를 박아 둔다.

          (1) 사용 카테고리는 노출도 축 5개로, 허용 3개(일반 의류, 액티브웨어,
              홈웨어·잠옷)와 제한 2개(속옷, 수영복)다. brandUseCategories.js와
              서버가 같은 풀을 사용한다(facemarket.py `_clean_uses`).
              같은 페이지의 description·라이선싱 카드도 '품목'이다.
