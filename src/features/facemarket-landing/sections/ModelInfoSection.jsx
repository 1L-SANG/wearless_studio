...
const RULES = [
  {
    ...
    // '공개 주소를 갖지 않고' → '공개 주소가 없고'. 앞은 have 구문을 그대로 옮긴 번역투이고,
    // 한국어는 소유가 아니라 존재로 쓴다. 바로 위 제목('공개 주소가 없어요')과도 표현이 맞물린다.
    // 뒤의 '다만 …'은 그대로 둔다 — 대표 이미지 예외를 지우면 이 문장이 거짓이 된다.
    body: '본인 확인에 올린 얼굴 사진은 공개 주소가 없고 권한이 확인된 요청에만 그때그때 열립니다. 다만 셀러 카탈로그에 걸리는 대표 이미지는 1시간만 유효한 서명 주소로 나가므로, 그 주소를 받은 사람은 그동안 로그인 없이 볼 수 있습니다. 대표 이미지는 건너뛸 수 있는 선택 단계입니다.',
  },
  {
    icon: 'eyeOff',
    ...
    // '렌더되지' → '그려지지'. DESIGN.md:309 "개발자 언어를 화면에 노출하지 않는다".
    // 주장 세기는 그대로다 — '그려지지 않는다'도 '아예 안 그린다'는 뜻이라, 숨겨 놓고
    // 안 보이게만 한 게 아니라는 원문의 강도를 잃지 않는다. '한 픽셀도'는 유지한다.
    body: '공개 검증 화면에는 라이선스 조건과 유효 여부, 그리고 마스킹된 이름과 나이가 나옵니다. 얼굴은 한 픽셀도 그려지지 않습니다.',
  },
  {
    icon: 'checkSquare',
