/* =============================================================
   ...
   서버가 내보내는 건 활동명·키·체형·라이선스 조건·공개 카탈로그 이미지뿐이고, 여기서도 그 이상을
   화면에 올리지 않는다(아래 toBrowseModel 이 필드를 골라 담는다 — 서버가 실수로 더 보내도 새지 않게).
...
export function heightText({ heightCm, heightBucket } = {}) {
  ...
  if (heightBucket) {
    const label = heightBucketLabel(heightBucket);
    if (label && label !== heightBucket) return label;
  }
  return null;
}
...
export function bodyTypeText(bodyType) {
  ...
  return label && label !== bodyType ? label : null;
}
...
function licenseView(license) {
  ...
  const uses = Array.isArray(license.allowedUse) ? license.allowedUse.filter(Boolean) : [];
 ...
 *   { id, kind: 'real'|'example', name, alt, closeup, fullbody|null, spec, body: [{dt, dd}], license: {uses, unitPrice, validity} }
 */
export function toBrowseModel(item) {
  ...
  const body = [];
  const height = heightText(item);
  if (height) body.push({ dt: '키', dd: height });
  const bodyType = bodyTypeText(item.bodyType);
  if (bodyType) body.push({ dt: '체형', dd: bodyType });
...
export function fromExampleModel(model) {
  return Object.freeze({
    ...
    body: [
      { dt: '키', dd: `${model.height}cm` },
      { dt: '몸무게', dd: `${model.weight}kg` },
    ],
    license: {
