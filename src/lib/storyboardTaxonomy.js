...
export function isWornCrossShotPair(cutType, sourceShot, targetShot) {
  // TODO(cross-shot polish): add another shot family only after its framing/expansion behavior
  // has real-generation evidence; unsupported pairs intentionally clear the selected example.
  return WORN_CUT_TYPES.has(cutType)
    && sourceShot !== targetShot
    && WORN_SHOTS.has(sourceShot)
    && WORN_SHOTS.has(targetShot);
}

export function generationExampleCompatible(example, {
  cutType, shot, clothingType, gender,
}, { allowCrossShot = false } = {}) {
  if (!example || example.cutType !== cutType) return false;
  if (example.shot !== shot
    && !(allowCrossShot && isWornCrossShotPair(cutType, example.shot, shot))) return false;
  if (!Array.isArray(example.applicableClothingTypes)
    || !example.applicableClothingTypes.includes(clothingType)) return false;
  return cutType === 'product' ? example.gender == null : example.gender === gender;
}

export function keepExampleOnShotChange(cutType, currentShot, nextShot) {
  return currentShot === nextShot || isWornCrossShotPair(cutType, currentShot, nextShot);
}

export function generationExampleStateAfterShotChange(
  cutType, currentShot, nextShot, { exampleId = null, refScope = 'all' } = {},
) {
  return keepExampleOnShotChange(cutType, currentShot, nextShot)
    ? { shot: nextShot, exampleId, refScope }
    : { shot: nextShot, exampleId: null, refScope: 'all' };
}

export function generationExampleScopeAvailable(
  variants, refScope, { inSpace = false, isProduct = false } = {},
) {
  const requiredScope = inSpace ? 'pose' : isProduct ? 'all' : refScope;
  return Array.isArray(variants) && variants.includes(requiredScope);
}
...
export function blockPatchForContentRole(block, role, { clothingType = 'top' } = {}) {
  ...
  if (!template.cutType) {
    return {
      contentRole: CONTENT_ROLES.CUSTOM, title: template.label,
      exampleId: null, refScope: 'all',
    };
  }
...
export function assignInternalContentRoles(blocks) {
  ...
  const next = blocks.map((block) => {
    ...
    const orphanedReferenceScope = !block.exampleId
      && (block.refScope === 'pose' || block.refScope === 'bg');
    if (!recipeChanged && !roleChanged && !productStateChanged && !orphanedReferenceScope) return block;
    ...
    return {
      ...
      ...(orphanedReferenceScope ? { refScope: 'all' } : {}),
      ...(recipeChanged ? {
        exampleId: null,
        refScope: 'all',
